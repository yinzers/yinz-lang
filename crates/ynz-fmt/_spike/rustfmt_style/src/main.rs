/// Rustfmt-style Yinz spike — preserves single-line forms when the original was single-line.
///
/// SPIKE ONLY — research code. Decision: PRETTIER-STYLE was chosen (see MEASUREMENTS.md).
/// This file is preserved in git history so the LOC comparison (376 rustfmt vs 421 prettier)
/// is auditable. Deleted in Phase 1's second commit.
///
/// Key difference from prettier_style: for function signatures, if the ORIGINAL source was
/// single-line, keep it single-line regardless of rendered length. This preserves user intent
/// but violates Yinz's "one canonical output" mandate — two authors writing the same 103-char
/// function on different lines get different formatted outputs.
...
fn emit_type(ty: &Type) -> String {
    match ty {
        ...
        Type::Union { variants, .. } => variants.iter().map(emit_type).collect::<Vec<_>>().join(" | "),
        ...
        Expr::InterpolatedString(parts, _) => {
            ...
            for part in parts {
                match part {
                    ...
                    StringPart::Expr(expr, _) => s.push_str(&format!("${{{}}}", emit_expr(expr))),
                }
            }
            s.push('`');
            s
...
fn emit_stmt(s: &Stmt, indent: usize, comments: &[(usize, String)], src: &str) -> String {
    ...
    match s {
        ...
        Stmt::Assign { target, value, .. } => format!("{ind}{target} = {}{inline}", emit_expr(value)),
        Stmt::Return { value, .. } => {
...
fn emit_block(stmts: &[Stmt], indent: usize, comments: &[(usize, String)], src: &str, block_start: usize) -> String {
    ...
    let mut prev_end: usize = block_start;
    for stmt in stmts {
...
fn emit_function(f: &FunctionDecl, comments: &[(usize, String)], src: &str) -> String {
    ...
    // RUSTFMT-STYLE: check whether the ORIGINAL param list was single-line.
    // If yes, keep it single-line regardless of rendered length.
    // This preserves user intent but breaks canonicality.
    let original_sig = src.get(f.span.start..f.span.end).unwrap_or("");
    let original_was_single_line = original_sig.find('{')
        .map(|i| !original_sig[..i].contains('\n'))
        .unwrap_or(false);

    let sig = if original_was_single_line {
...
fn format_source(source: &str) -> String {
    ...
    for item in &module.items {
        ...
        for ct in &leading { out.push_str(ct); out.push('\n'); }
        let item_s = match item {
            Item::Function(f) => emit_function(f, &comments, source),
            Item::ShapeDecl(s) => emit_shape(s, &comments, source),
            Item::ImportDecl(i) => source.get(i.span.start..i.span.end).unwrap_or("").trim().to_string(),
            Item::ConstDecl(c) => format!("const {} = {}", c.name, emit_expr(&c.value)),
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("usage: ynz-fmt-spike-rustfmt <file.ynz>");
        process::exit(2);
    }
    let path = &args[1];
