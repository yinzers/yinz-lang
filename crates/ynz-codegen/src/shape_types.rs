...
pub fn emit_shape_types<'ctx>(ctx: &'ctx Context, shape_table: &ShapeTable) -> ShapeLlvmTypes<'ctx> {
    let ptr = ctx.ptr_type(AddressSpace::default());
    let fat_ptr = ctx.struct_type(&[ptr.into(), ptr.into()], false);

    let mut named = HashMap::new();
    for (shape_name, shape_def) in &shape_table.shapes {
        let field_types: Vec<BasicTypeEnum<'ctx>> = shape_def.fields.iter().map(|f| {
            llvm_field_type(ctx, &f.ty)
        }).collect();
