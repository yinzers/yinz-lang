...
fn check_no_diags(source: &str) -> CheckOutput {
    ...
    assert_eq!(
        out.diagnostics.len(), 0,
        "Expected no diagnostics, got: {:#?}", out.diagnostics
    );
    out
}
...
fn check_diag_count(source: &str, expected: usize) -> CheckOutput {
    let out = check(source);
    assert_eq!(
        out.diagnostics.len(), expected,
        "Expected {expected} diagnostic(s), got: {:#?}", out.diagnostics
    );
    out
}
...
"#);
    let entries: Vec<_> = out.mono_table.entries.keys()
        ...
        .collect();
    assert_eq!(entries.len(), 2, "Expected two identity instantiations (int + string)");
}
...
"#);
    let entries: Vec<_> = out.mono_table.entries.keys()
        .filter(|k| k.fn_name == "identity")
        .collect();
    assert_eq!(entries.len(), 1, "Same type args must dedup to one table entry");
}
...
"#);
    ...
    assert!(
        ...
        "Error must mention the contract name `Comparable`, got: {:?}", diag
    );
}
...
"#);
    assert_eq!(out.diagnostics.len(), 1, "const-cannot-be-given must still fire");
}
...
"#);
    ...
    assert_eq!(sig.ret_type, Type::Int, "Monomorphized identity<int> must have ret_type = int");
}
...
"#);
    let key = out.mono_table.entries.keys()
        ...
        .unwrap();
    let sig = out.mono_table.entries.get(key).unwrap();
    assert_eq!(sig.param_types, vec![Type::Int], "Monomorphized identity<int> must have param_type = int");
}
