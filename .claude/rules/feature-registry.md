...
> **Every new user-facing feature item goes in `registry/features.toml` first. Code follows the registry — the registry does not follow the code.**

See `design/feature-registry.md` for the full schema reference and carve-out policy.
...
| New IDE muted-hint inference domain | `[[muted_hint_domain]]` | `placement_category` must match `.claude/rules/inference.md` |
...
The `.claude/graveyard.md` "scattered-registry-without-SSOT" entry runs a grep check on every diff. It catches new `pub const`/`pub static` string-array definitions in `crates/ynz-{diagnostics,typeck,parser}/src/` that lack a `// CARVE-OUT:` annotation. If the Bouncer fires, either:
...
- `design/feature-registry.md` — full schema + carve-out policy + self-hosting migration
- `registry/features.toml` — the TOML data file (source of truth)
- `crates/ynz-registry/` — the crate that parses TOML + generates Rust
- `.claude/graveyard.md` — "scattered-registry-without-SSOT" Bouncer entry
- `.claude/rules/plan-invariants.md` — `### Feature Registry Entries` subsection (required for v0.2-M2+ plans)
