...
> **A plan is a route to the design, never an override of it.** Before a plan is approved, it MUST be checked against the governing `docs/internal/implementation/IMP-*.md` docs — especially `docs/internal/scratchpad/SCRATCH-future-*.md` for end-state vision.

Every plan MUST include a **`## Design-Doc Alignment`** section that:

1. **Cites the governing design doc(s)** the plan builds toward (e.g. [`docs/internal/implementation/IMP-no-function-coloring.md`](../../docs/internal/implementation/IMP-no-function-coloring.md), [`docs/internal/implementation/IMP-type-system.md`](../../docs/internal/implementation/IMP-type-system.md)). "None — no design doc covers this" is an allowed answer ONLY after confirming `docs/internal/implementation/` truly has no relevant file; if it doesn't, that's a signal the design should be written first. **Verify each citation for specification DEPTH at recon time, not just topical relevance** — read the cited doc's actual text and confirm it genuinely *specifies* the mechanism you are citing it for (the exact sharing topology, read convention, cancellation behavior, ABI shape), rather than merely *mentioning* the topic. A citation that only names the subject leaves the plan silently un-anchored and defers the real design question to mid-execution: v0.3-M4 discovered mid-flight, three separate times, that a cited design doc was silent on precisely what it was cited for (Arc sharing topology; the background-handle child-side read convention; a stale Task-Cancellation claim that survived unchallenged to the final completion gate). If the cited doc is silent on the mechanism, that silence IS the recon finding — surface it and get the design written or the citation corrected; do not carry a topical citation as if it settled a question its text never answered.
...
4. **Verifies every behavior claim about pre-existing code the plan's own phases will NOT touch.** An invariant / Safety / Kernel-Mode assertion about how existing code *already* behaves (e.g. "kernel mode declines mixed groups to sequential") is a claim to CONFIRM by reading or grepping the actual code at recon time — not to infer and defer to execution. These checks are cheap (one grep/read) and a wrong one surfaces mid-execution phases later as a costly correction. A behavior claim about adjacent-but-untouched code carried without a recon citation is a plan defect, same class as an un-surfaced design divergence.
...
- New executable feature → add to `examples/pirates-roster/entrypoint.ynz` showing the feature in a small but realistic context (not just `print(featureName())` — show it doing real work)
...
- Verification is **byte-exact, not `insta`, for these two files** (established M3-series precedent — M3b/M3d/M3e/M3f). The demo (`examples/pirates-roster/entrypoint.ynz`) is checked against the committed `examples/pirates-roster/expected_stdout.txt` golden via `assert_eq!(stdout, golden)` in `crates/ynz-driver/tests/integration.rs`; regenerate the golden with `examples/pirates-roster/expected_stdout.txt.regenerate.sh` when the demo changes intentionally. The `m{N}_errors.ynz` gallery is checked in `crates/ynz-driver/tests/error_galleries.rs` by diagnostic-count + key-phrase assertions. (`insta` snapshots are used elsewhere in the suite, but the demo + gallery use the byte-exact golden / count+phrase convention — do not reach for `insta` for these two.)
...
- [`.claude/graveyard.md`](../graveyard.md) "Const Deep-Immutability Invariant Unstated in Milestone Plans" catches M4+ plans missing the const-deep-immutability invariants in `### Safety`
- [`.claude/graveyard.md`](../graveyard.md) "M4+ Milestone Plans Missing the 5-Subsection Invariants Structure" catches M4+ plans missing any of the (now 7) required sub-sections
- [`.claude/graveyard.md`](../graveyard.md) "Language or Stdlib Features Without Runtime + Kernel-Mode Declaration" catches plans that touch `crates/**` without declaring runtime dependencies and kernel-mode behavior
...
- [`.claude/graveyard.md`](../graveyard.md) ("Const Deep-Immutability Invariant Unstated in Milestone Plans," "M4+ Milestone Plans Missing the 5-Subsection Invariants Structure," and "Language or Stdlib Features Without Runtime + Kernel-Mode Declaration" enforce this rule)
