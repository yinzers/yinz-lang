...
- [ ] **ci-clippy-cleanup-v0.3.0-m3 (2026-06-01, deferred per Patrick "CI is for later")** — `cargo clippy --workspace -- -D warnings` (CI gate, `.github/workflows/ci.yml:56`) is RED. Plain `cargo build` is fine (warnings only); these are `-D warnings` denials, all ~1-line fixes: (1) `crates/ynz-lsp/src/server.rs:88` — deprecated `InitializeParams::root_uri` (PRE-EXISTING from M10; the `root_path` fallback 6 lines below already has `#[allow(deprecated)]` — add the same; we intentionally read both deprecated LSP fields for editor compat). (2) `crates/ynz-typeck/src/check.rs:44` — `too_many_arguments (8/7)` on `check()` (v0.3-M2 added the 8th param `imported_fn_names`, used at :1842 for the cross-module suspension gate — add `#[allow(clippy::too_many_arguments)]` per the convention in 9+ spots incl. `check.rs:109`). (3) `check.rs:4914` + (4) `check.rs:4977` — `collapsible_if` (collapse the `if` into the outer `match` guard; both in the v0.3-M2 locals-crossing-wait scan). Note: origin/main CI has been red since the M10 release (`root_uri` alone), so this also un-breaks pre-existing CI.
