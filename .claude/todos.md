...
*(main = origin/main + repo-cleanup, synced. **v0.3.0-m2 SHIPPED** = M10 teaching-surface bug hunt (was thought unmerged — local was 19 commits stale; M10 is DONE). v0.2.1 LSP work is SERIAL now (no parallel worktrees). Only live workstream branch: `v0-3-m2-wait-and-state-machines` (concurrency milestone, in flight — NOT the v0.3.0-m2 release, which is M10). webpage branch parked.)*

- [ ] **decide-webpage-branch (2026-05-31)** — `feat/webpage-foundation-scaffold` (PR #65 open, ~50 commits behind, worktree pruned): rebase onto current main or scrap? Parked until decided.
