...
- [2026-05-12] **M2 Phase 4 complete (feat/m2-typeck, awaiting review+commit)**: Full M2 type checker. `builtins.rs` → `intrinsics.rs` (git mv); `PrimitiveIntrinsicTable` (polymorphic print + 8 conversion methods); `scope.rs` with Levenshtein suggestion; literal-hint inference; mixed-type arithmetic errors with specific `.toX()` per direction; spec corrections (variables.md + numeric-types.md). Bouncer caught + fixed test weakening in `m1_source_type_checks_clean`. 38 typeck tests. Next: user reviews, then commit + merge + Phase 5 codegen on `feat/m2-codegen`.
