...
4. **Lint rules** — [`docs/internal/implementation/IMP-linting.md`](../../../../docs/internal/implementation/IMP-linting.md) is fully specced, the protocol exists, and the two auto-promotion analyses (`array_to_fixed_promotion`, `let_to_const_promotion`) already run for inlay hints. The tier-3 yellow squiggle just needs to be emitted.
...
- **Plan invariants stay in force.** Per [`.claude/rules/plan-invariants.md`](../../../rules/plan-invariants.md), every milestone execution plan from v0.2-M2 onward must include the seven required Invariants subsections including `### Feature Registry Entries`. v0.2.1 milestones are no exception.
...
- **No new lint rules beyond the 2 starter rules in v0.2.1.** Other rules from [`docs/internal/implementation/IMP-linting.md`](../../../../docs/internal/implementation/IMP-linting.md) ("repeated-inline-shape", "unused-import-found", etc.) are explicitly deferred. The 2 starter rules ship the *infrastructure* — adding rule #3 in v0.3+ should be a single registry entry plus an analysis pass, not a milestone-scale effort.
...
- **M8: Modifier mapping is rule-driven by registry + typeck.** `DEPRECATED` ← name appears in `[[banned_jargon]]` or `[[deferred_language_feature]]`. `READONLY` ← typeck classifies binding as `const`. `MODIFICATION` ← typeck classifies binding as `lend` parameter at this site (matches the cautionary red-tinted muted hint per [`.claude/rules/inference.md`](../../../rules/inference.md)). `DEFINITION` ← span is a declaration site, not a use site. `DEFAULT_LIBRARY` ← name resolves to a primitive intrinsic (`toString`, `byteAt`) or a type-attached constant (`int.max`). `STATIC` ← type-attached constant only (subset of `DEFAULT_LIBRARY`, distinguishable in themes).
...
- **P6 — Muted-hint domain description fixes**: replace banned `infer`/`inferred` in 3 `[[muted_hint_domain]]` description fields with "figures out" or "the compiler picks" per [`.claude/rules/vocabulary.md`](../../../rules/vocabulary.md).
...
- **User-defined lint rules (plugin system).** Documented in [`docs/internal/implementation/IMP-linting.md`](../../../../docs/internal/implementation/IMP-linting.md) as a v1.x+ surface. Out of scope for any v0.x milestone.
...
- **`inlineValue` provider.** Blocked by the debug adapter (v0.5+, no design doc yet). Tracked in [`docs/internal/implementation/IMP-lsp.md`](../../../../docs/internal/implementation/IMP-lsp.md) "Capability Inventory" table.
- **`moniker` provider.** Blocked by the package manager (v0.22). Cross-repo navigation requires per-package symbol monikers. Tracked in [`docs/internal/implementation/IMP-lsp.md`](../../../../docs/internal/implementation/IMP-lsp.md).
- **Notebook controller.** No Yinz notebook format exists. Not planned for any current milestone.
- **`workspace/didChangeWatchedFiles` server-side handler.** Today the extension creates the watcher but the LSP server doesn't subscribe. Ships when v0.22 package manager needs lock-file change detection OR when external file edits (CLI formatter, code-mod scripts) need to invalidate salsa inputs. Tracked in [`docs/internal/implementation/IMP-lsp.md`](../../../../docs/internal/implementation/IMP-lsp.md).
- **Pull-diagnostics (`textDocument/diagnostic`).** Already deferred per registry `lsp-pull-diagnostics` entry.
- **`codeDescription` URI on diagnostics.** Already deferred per [`docs/internal/implementation/IMP-lsp.md`](../../../../docs/internal/implementation/IMP-lsp.md) — ships when the docs site lands.
- **Refactor code action kinds (`SOURCE_ORGANIZE_IMPORTS`, `REFACTOR_EXTRACT`, `REFACTOR_INLINE`).** Blocked by the refactor catalog (need import-sorter for organize, extract-function pass for extract). Tracked in [`docs/internal/implementation/IMP-lsp.md`](../../../../docs/internal/implementation/IMP-lsp.md) "Capability Inventory" table.
