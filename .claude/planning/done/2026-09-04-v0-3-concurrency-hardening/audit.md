...
### Close-out End-State verdict — two blockers found IN THE CLOSED PLAN ITSELF, one producer

`plan-adherence-high`, dispatched over the WHOLE plan against §3.1 rather than over one round, is
what caught them. The per-phase gates structurally could not: each one asked "did this round do what
this round said," and no round ever claimed the stale text.

- **Blocker — `### Safety` claimed two invariants ON THE STRENGTH OF PHASE 4**, which never ran. The
  two pin tests it cited as PROOF currently assert the exact opposite (they pin today's no-release
  behaviour and pass). A closed plan handing its successor a safety floor it never poured is the
  worst version of this defect, because M9 would inherit it as settled.
- **Blocker — `### Demo & Error Gallery` asserted a handle-cancellation demo exists** in
  `examples/pirates-roster/entrypoint.ynz`. Verified absent by grep. It could not have existed: the
  behaviour it describes is unimplemented.
- **should-fix ×2** — `## Design-Doc Alignment` (three claims) and `### 3.1 Intent & End State`
  (Purpose, key task 4, "what done looks like") all still spoke as though Phase 4 ran here.

**Producer, named once rather than patched three times** (`root-cause.md`): the close-out edit pass
was scoped to the sections the sizing and defer dispatches TOUCHED — banner, Phase 4 section, Future
Requirements, Feature Registry Entries — and never swept the plan for OTHER Phase-4-dependent claims.
One incomplete sweep, eight downstream instances. The fix was a single sweep over every `Phase 4`
occurrence in the file, correcting all eight in one operation; each corrected claim is struck through
and marked rather than deleted, so a reader sees what was claimed AND that it did not happen.

Not tagged `frago-needed`: the actual leak/UAF risk is unchanged and was already named in §1's risk
table. This was a documentation-honesty defect in the closed artifact, not new system risk.

Also confirmed by that seat, independently: the M9 first-commit claim holds — no existing test pins
"an ordinary `array`/`map` local is never freed at scope exit" (the nearest, `alloc_counter_shape_crossing_local_no_leak`
and siblings, check frame-allocation balance, a different question). And the seat declined to relay
Phase 3's "135 targets, zero failures" as verified fact, since `cargo` was absent from its PATH —
recorded here as the correct behaviour, not a gap.
