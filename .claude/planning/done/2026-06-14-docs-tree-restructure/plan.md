...
metadata:
  type: "plan"
superseded_by: "direct execution, 2026-07-01 (no new plan-id — see note below)"
legacy:
...
> **⚠️ SUPERSEDED 2026-07-01 — do not execute.** This plan's execution precondition (m3d done) is now satisfied, but the plan itself targets a DEAD global standard: `~/.claude/rules/documentation.md`, `~/.claude/memory/doc-style.md`, `~/.claude/memory/doc-types.md` (all cited below) no longer exist on this machine — they were consolidated into `~/.claude/docs/internal/implementation/IMP-documentation-system.md` (created 2026-06-25) sometime after this plan was drafted (2026-06-14/15). This plan's target structure (`docs/user/` + `docs/internal/{architecture,decisions}`, `doc-type:` frontmatter tags) does NOT match the current standard's structure (`docs/{internal/{decisions,implementation,scratchpad,...},reference,user-facing}` with `ADR-`/`IMP-`/`REF-`/`SCRATCH-`/`TUT-` filename prefixes). **The restructure this plan describes was executed directly in a live session on 2026-07-01** (Patrick's explicit instruction, bypassing the formal plan pipeline given the session was already deep in a related doc-audit-and-fix pass) against the CURRENT standard: all 110 `design/`+`spec/` files moved into `docs/` with the ADR/IMP/REF/SCRATCH taxonomy, frontmatter added to every file, and every repo cross-reference rewritten. This plan's own classification proposal (spec→locked user docs vs aspirational→`docs/internal/ideas/`) was NOT what shipped — the actual mapping used was: design/root → mostly IMP (a few genuine one-time calls → ADR), design/open-questions.md + design/future/** + design/stdlib/** → SCRATCH, spec/** → REF (Patrick's call: spec content reads as reference material, not step-by-step tutorials, so it earned REF over `docs/user-facing/tutorials/`). No `docs/internal/ideas/` bucket was created — SCRATCH already covers "not yet built" content in the current standard. **If a future session wants the maturity-split (locked-vs-aspirational) idea this plan proposed, that's still real, undone work — but it needs a FRESH plan against the current standard, not a resume of this one.**
...
- [`docs/README.md`](../../../../docs/README.md) (index-only), [`docs/internal/scratchpad/SCRATCH-open-questions.md`](../../../../docs/internal/scratchpad/SCRATCH-open-questions.md), [`docs/reference/REF-language-overview.md`](../../../../docs/reference/REF-language-overview.md), [`docs/internal/scratchpad/SCRATCH-future-designs-index.md`](../../../../docs/internal/scratchpad/SCRATCH-future-designs-index.md) are the closest things to navigation today.
- [`.claude/design-sources.md`](../../../design-sources.md) globs `[locked] design/**/*.md` and `[locked] spec/**/*.md` — the design-compliance gate depends on these paths.
...
- [`docs/README.md`](../../../../docs/README.md) is a navigation index / decision-tree labeling internal vs user vs the doc-type buckets.
...
- [`.claude/design-sources.md`](../../../design-sources.md) globs updated to `docs/**`; the design-compliance gate still resolves.
...
- **Cross-reference blast radius = 126 files** reference `design/`/`spec/` paths:
  ...
  - `.claude/rules/` + [`CLAUDE.md`](../../../../CLAUDE.md): 13 files.
...
- **[`docs/README.md`](../../../../docs/README.md) is already index-only** — it becomes raw material for [`docs/README.md`](../../../../docs/README.md) + the `docs/internal/decisions/` ADR index.
...
| 3 | `design-sources.md` globs `design/**` + `spec/**` as `[locked]`; gate depends on them | verified | [`.claude/design-sources.md`](../../../design-sources.md) lines: `- [locked] spec/**/*.md`, `- [locked] design/**/*.md` |
| 4 | `concurrency.md` carries garden-path dated decision sections (ADR candidates) | verified | [`docs/internal/implementation/IMP-concurrency.md`](../../../../docs/internal/implementation/IMP-concurrency.md) headings: `## Suspension … (LOCKED 2026-06-05)`, `## Design Divergences (v0.3-M3b …)` |
...
| [`docs/README.md`](../../../../docs/README.md) — navigation index / decision-tree (internal vs user vs doc-type buckets) | Phase 6 | doc-type: overview |
...
- [`.claude/rules/docs-checklist.md`](../../../rules/docs-checklist.md) — the stale project rule this replaces
- `~/.claude/rules/documentation.md:155` — §g project-extension mechanism to cite
- [`docs/README.md`](../../../../docs/README.md) — index-only; becomes raw material for the decisions/ADR index
**Files (expected scope)**: `.claude/rules/documentation.md` (new), [`.claude/rules/plan-invariants.md`](../../../rules/plan-invariants.md) (lifecycle requirement), [`.claude/design-sources.md`](../../../design-sources.md) (policy text amend), `docs/` (new empty dirs incl. `ideas/`), this plan file (mapping table appended)
...
1. Write `.claude/rules/documentation.md`: `Extends: ~/.claude/rules/documentation.md`. Encode: Yinz layout (`docs/user/` = user-facing locked specs, `docs/internal/architecture` + `docs/internal/decisions` = compiler, **`docs/internal/ideas/` = aspirational idea-docs**), the **Diátaxis-split-at-feature-lock** rule (Decision #7), `doc-type` + **`status: aspirational | locked`** frontmatter requirement, the **doc lifecycle** (an idea-doc in `docs/internal/ideas/` must be designed+locked when its feature is planned, then deleted — or trimmed if only some of its ideas shipped), and retirement of `docs-checklist.md` (checklists fold in, repointed). Do NOT re-spec §a–§f of the global rule — cite it.
   - Also amend [`.claude/design-sources.md`](../../../design-sources.md)'s 2026-06-13 policy text: un-retire the `[aspirational]` tier **for `docs/internal/ideas/**` only**; all `/design/` docs and shipped-feature user specs remain `[locked]`.
   - Add a `### Doc Lifecycle` enforcement requirement to [`.claude/rules/plan-invariants.md`](../../../rules/plan-invariants.md): any plan that locks a feature whose spec lives in `docs/internal/ideas/` MUST include a step that designs+locks the real doc and deletes/trims the idea-doc (plan-reviewer checks plan-invariants → mechanical enforcement).
...
**PR scope**: `git mv` all `spec/` → `docs/user/` and `design/` → `docs/internal/…` per Phase 1's mapping, AND rewrite every `design/`/`spec/` path reference repo-wide (internal links, rules, [`CLAUDE.md`](../../../../CLAUDE.md), 55 crate doc-comments, `examples/`, `registry/`, active plans, `design-sources.md` globs). Pure path rewrite — zero content/tag/prose change.
...
**Current-state anchors**:
- [`.claude/design-sources.md`](../../../design-sources.md) — `[locked] design/**/*.md` + `[locked] spec/**/*.md` globs → become `docs/**`
- `crates/**` — 55 files with `design/…md` doc-comment refs
- [`CLAUDE.md`](../../../../CLAUDE.md) Project Layout table — references `/spec/` and `/design/`
**Files (expected scope)**: `git mv` of `spec/**`+`design/**`; ref edits in `crates/**`, `.claude/rules/**`, [`CLAUDE.md`](../../../../CLAUDE.md), `examples/**`, `registry/**`, `.claude/plans/active/**`, [`.claude/design-sources.md`](../../../design-sources.md), and the moved docs' internal links.
...
3. Update [`.claude/design-sources.md`](../../../design-sources.md): replace `design/**` + `spec/**` globs with `[locked] docs/internal/architecture/**`, `[locked] docs/internal/decisions/**`, `[locked] docs/user/**`, and **`[aspirational] docs/internal/ideas/**`**; repoint the high-stakes per-doc callouts (concurrency, no-function-coloring, no-runtime-mode, ide-hints).
4. Update [`CLAUDE.md`](../../../../CLAUDE.md) Project Layout table rows to the `docs/` paths.
**Acceptance criteria**:
...
**Verification**: the zero-dangling-ref grep; `cargo build --workspace`; spot-read 3 crate files + 3 moved docs for correct new paths. **Adversarial checks (plan-reviewer):** (a) nested-path correctness — confirm [`docs/internal/scratchpad/SCRATCH-future-gui-architecture.md`](../../../../docs/internal/scratchpad/SCRATCH-future-gui-architecture.md) landed at the full nested dest, NOT a flattened `…/future.md`; the rewrite is driven key-by-key off the Phase 1 mapping table, never a blanket regex; (b) cross-link target resolution — trace at least one chain end-to-end (e.g. `decisions.md` → `concurrency.md`) and confirm the rewritten link resolves to the target's NEW home.
**Phase Review Gates**:
- [ ] code-reviewer: <verdict + ISO timestamp>
- [ ] rules-compliance-reviewer: <verdict + ISO timestamp>
- [ ] plan-adherence-verifier: <verdict + ISO timestamp>
...
**Current-state anchors**:
- [`docs/internal/implementation/IMP-concurrency.md`](../../../../docs/internal/implementation/IMP-concurrency.md) → `docs/internal/architecture/concurrency.md` — `LOCKED 2026-06-05` headings + `## Design Divergences (M3b)` section
...
**Current-state anchors**:
- [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) — source of truth for "is feature X in v0.N / shipped?"
...
**PR scope**: Write [`docs/README.md`](../../../../docs/README.md) (the decision-tree / map: internal vs user, doc-type buckets, "start here") and `docs/overview.md` (board-altitude project explainer). Fold [`docs/README.md`](../../../../docs/README.md) + [`docs/reference/REF-language-overview.md`](../../../../docs/reference/REF-language-overview.md) content into the new index/overview as appropriate.
...
**Current-state anchors**: [`docs/README.md`](../../../../docs/README.md) (→ moved), [`docs/reference/REF-language-overview.md`](../../../../docs/reference/REF-language-overview.md) (→ moved), `~/.claude/memory/doc-style.md` (house style for the index/overview).
**Files (expected scope)**: [`docs/README.md`](../../../../docs/README.md) (new), `docs/overview.md` (new); de-dupe the old index files.
**Deviation rule**: standard.
**Steps**:
1. Write [`docs/README.md`](../../../../docs/README.md): TL;DR table, the internal/user/doc-type map, a "start here" decision-tree, links to the ADR index. Follow `doc-style.md` (progressive disclosure, present-tense).
...
**Acceptance criteria**:
- [ ] [`docs/README.md`](../../../../docs/README.md) labels internal vs user + doc-type buckets and has a "start here" path
  - Evidence: (filled at phase completion)
...
**Verification**: read [`docs/README.md`](../../../../docs/README.md) — can a newcomer find internal vs user vs an ADR in <2 hops?
**Phase Review Gates**:
- [ ] code-reviewer: <verdict + ISO timestamp>
- [ ] rules-compliance-reviewer: <verdict + ISO timestamp>
- [ ] plan-adherence-verifier: <verdict + ISO timestamp>
