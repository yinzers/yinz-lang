---
name: "m3-control-flow-fns"
plan-id: "2026-05-13-m3-control-flow-fns"
status: "done"
...
- **`for (item in items)` and `if (cond)` parens are required.** Per spec — both forms wrap their head in parens. This matches M1's `function entrypoint() -> nothing { ... }` paren-required style. Parser rejects `if cond { }` without parens.
...
11. Tests:
    ...
    - **Undefined function**: `function entrypoint() -> nothing { unknownFn() }` → three-part with Levenshtein.
    - **Function arg type mismatch**: `function foo(x: int) -> int { return x }; function entrypoint() -> nothing { foo("hi") }` → three-part.
    ...
    - **Adversarial: nested call expressions**: `function add(a: int, b: int) -> int { return a + b }; function entrypoint() -> nothing { print(add(add(1, 2), add(3, 4))) }` — exercises nested call type-check (each call arg is itself a call). Must type-check and emit `10\n`.
