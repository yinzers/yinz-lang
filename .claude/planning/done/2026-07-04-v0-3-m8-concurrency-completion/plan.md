...
> ### Phase 7 STATUS — steps 1–2 ✅ recon done, **CHECKPOINT reached and the decision made: Branch B (RE-DEFER)**, step 4's four-field re-deferral AUTHORED 2026-09-04 (executor `m8-p7-20260904-a1`; nothing committed) — **SIGNED BY PATRICK 2026-09-04** (step 4's own gate, now MET) — ruling (i): the re-deferral is accepted AND the cheap guard is taken, as a **Tier 3 lint** rather than the muted hint originally proposed, because he prefers a known deferred defect to nag loudly rather than sit in passive gray text. Shipped in `e38e19a` as `[[lint_rule]] background-handle-not-waited`. He further ruled the underlying defect a PRIORITY fix rather than an open-ended deferral, on a case this phase's own framing had missed: in a long-lived server `entrypoint` never returns, so the program-exit sweep that rescues a short script never fires and unstoppable tasks accumulate. Plan `2026-09-04-v0-3-concurrency-hardening` Phase 4 owns the fix and retires the lint; step 5 (full suite) is the gate agent's lane
>
...
- **`background-handle-cancel-injection`** (existing entry): retired (Phase 7 Branch A) if the
  ...
  **Outcome 2026-09-04: Branch B — `modify [[deferred_language_feature]] name =
  ...
  version"` — **SIGNED by Patrick 2026-09-04**, ruling (i) of the three offered: sign the re-deferral AND take the guard. His words: "lets do a muted hint for now" — then, on being shown that the deferred behavior nags nobody: "i dont like shipping silent errors i prefer loud ones that remind us we need to fix it," which converts the guard from a muted hint to a **Tier 3 lint** (editor squiggle AND compile output), firing only on the exposing shape — a bound handle whose scope ends with no `wait` on it. He also ruled the underlying defect a PRIORITY fix rather than an indefinite deferral, on the grounds that a task nobody can stop is a leak in a long-lived server where `entrypoint` never returns: plan `2026-09-04-v0-3-concurrency-hardening` Phase 4 owns it and retires this lint. (Future Requirements #3).**
...
3. **Track 3 re-deferral — Phase 7 took Branch B (RE-DEFER), 2026-09-04, executor
   `m8-p7-20260904-a1`; **SIGNED by Patrick 2026-09-04**, ruling (i) of the three offered: sign the re-deferral AND take the guard. His words: "lets do a muted hint for now" — then, on being shown that the deferred behavior nags nobody: "i dont like shipping silent errors i prefer loud ones that remind us we need to fix it," which converts the guard from a muted hint to a **Tier 3 lint** (editor squiggle AND compile output), firing only on the exposing shape — a bound handle whose scope ends with no `wait` on it. He also ruled the underlying defect a PRIORITY fix rather than an indefinite deferral, on the grounds that a task nobody can stop is a leak in a long-lived server where `entrypoint` never returns: plan `2026-09-04-v0-3-concurrency-hardening` Phase 4 owns it and retires this lint. The phase's own exit criterion is met.** Evidence:
