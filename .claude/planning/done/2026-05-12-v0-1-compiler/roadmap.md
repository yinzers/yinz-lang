---
name: "v0-1-compiler"
plan-id: "2026-05-12-v0-1-compiler"
status: "done"
roadmap-id: null
session-id: []
...
**Status**: COMPLETE (2026-05-13) — on `feat/m2-codegen`, awaiting user review + commit. All workspace tests green. Key decisions and bugs found: (1) `runtime_decls.rs` — single struct holding all extern C declarations; (2) `emit.rs` fully rewritten — `let`/assign lowering with alloca+store, int overflow via `llvm.sadd/ssub/smul.with.overflow.i64`, float native ops, decimal128 via runtime, short-circuit `&&`/`||` with phi nodes, polymorphic `print` lowering per type, conversion method dispatch; (3) `expr_types` key **bug fix** — changed from `usize` (span.start) to `(usize, usize)` (span.start, span.end): a BinOp's span.start equals its leftmost child's span.start, so the parent's type was overwriting the child's in the HashMap — caused "unsupported binop Gt Bool" for `count > 0` inside `active && (count > 0)`; (4) `ValueKind::basic()` not `.left()` in inkwell 0.9; (5) `ynz_numerics::parse(s) -> Option<u128>` not `Decimal128::parse()`.
