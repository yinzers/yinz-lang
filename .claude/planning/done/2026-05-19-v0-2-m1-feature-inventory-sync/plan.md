---
name: "v0-2-m1-feature-inventory-sync"
plan-id: "2026-05-19-v0-2-m1-feature-inventory-sync"
status: "done"
roadmap-id: "2026-05-18-v0-2-dev-loop-tooling"
session-id: []
...
legacy:
  ...
  files:
    ...
    - docs/internal/implementation/IMP-feature-registry.md
...
- **Implementation shape**: data file (TOML) at [`registry/features.toml`](../../../../registry/features.toml), parsed by a `build.rs` in `crates/ynz-registry/` that generates Rust code in `OUT_DIR`. Consumers `include!()` the generated file or import via the `ynz-registry` crate's public API.
...
- **All compile errors continue to follow WHAT/WHAT-INSTEAD/WHY** per [`docs/reference/REF-teaching-mission.md`](../../../../docs/reference/REF-teaching-mission.md) — registry-driven errors render through the same `Diagnostic` constructor.
...
**Success criteria**:
- Every existing scattered registry has migrated to [`registry/features.toml`](../../../../registry/features.toml). Auditing "what feature surfaces does crate X consume?" has a single grep-able answer per crate.
...
- A new project rule ([`.claude/rules/feature-registry.md`](../../../rules/feature-registry.md)) + plan-invariants subsection (`### Feature Registry Entries`) forces every v0.2-M2-onward execution plan to list its registry additions.
...
| Self-hosting transition (v2+) requires re-implementing build.rs in Yinz; nothing today proves the Yinz stdlib can do what `toml` crate does | Low (timing) | Low | TOML parsing is in Yinz stdlib v0.22+ (per [`docs/internal/implementation/IMP-packages.md`](../../../../docs/internal/implementation/IMP-packages.md) — `yinz.toml` parsing already required). No M1 work needed; documented in [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) "Self-hosting migration plan" subsection as a recognized future task. |
...
- Incremental builds with no [`registry/features.toml`](../../../../registry/features.toml) change show ZERO overhead from the registry (`build.rs` declares `cargo:rerun-if-changed=registry/features.toml` and nothing else)
...
- Three NEW teaching surfaces shipped:
  - [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) — design rationale + schema reference + Carve-Outs section + self-hosting migration plan + "when to add an entry" + "when an entry is the wrong fit"
  - [`.claude/rules/feature-registry.md`](../../../rules/feature-registry.md) — project rule loaded on demand for any milestone touching the registry or adding a language feature
  - Updated [`.claude/rules/plan-invariants.md`](../../../rules/plan-invariants.md) `### Feature Registry Entries` subsection — mandatory for every plan from v0.2-M2 onward (this plan itself is exempt; it's the one creating the rule)
- The IDE muted-hint domain table is POPULATED in the registry (one entry per domain from [`.claude/rules/inference.md`](../../../rules/inference.md)) but has NO consumer yet — v0.2-M2 LSP wires the consumer. Populating now means M2 doesn't have to also populate them.
...
- `toml` crate dependency: BUILD-TIME ONLY (in [`crates/ynz-registry/Cargo.toml`](../../../../crates/ynz-registry/Cargo.toml) `[build-dependencies]`). Not a runtime dependency of the compiler binary.
...
### Demo & Error Gallery
- `examples/pirates-roster/entrypoint.ynz`: NO extension required. This milestone is a refactor with no new user-facing language feature. Per [`.claude/rules/plan-invariants.md`](../../../rules/plan-invariants.md) "Demo & Error Gallery": features that ship without user-facing surface don't need entrypoint extensions. State this explicitly in the Phase 8 PR description.
...
- `docs/reference/REF-mvp-scope.md:*` — current v2+ deferred-features prose (will get back-pointer to registry entries when those land in Phase 5b)
- `docs/internal/decisions/ADR-compiler-language.md:*` — current "Why Salsa" section (will get an "SSOT registry" paragraph)
...
- NEW [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) (~250 lines: schema reference, consumer API contract, Carve-Outs section, self-hosting migration plan, "when to add" / "when not to add")
- NEW [`.claude/rules/feature-registry.md`](../../../rules/feature-registry.md) (~80 lines: project rule loaded on demand)
- UPDATE [`.claude/rules/plan-invariants.md`](../../../rules/plan-invariants.md) (~+40 lines: new `### Feature Registry Entries` 7th subsection, marked "applies to plans from v0.2-M2 onward")
- UPDATE [`CLAUDE.md`](../../../../CLAUDE.md) (~+15 lines: new rule pointer in Project Layout + Rules Files tables)
- UPDATE [`.claude/graveyard.md`](../../../graveyard.md) (~+50 lines: new entry "scattered registry without SSOT link" with grep pattern)
- UPDATE [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) (~+20 lines: each "Locked design, deferred from v0.1" entry gets a placeholder line "Registry entry: TBD M1" — the actual entry-name lands in Phase 5b)
- UPDATE [`docs/internal/decisions/ADR-compiler-language.md`](../../../../docs/internal/decisions/ADR-compiler-language.md) (~+15 lines: new paragraph "Both also share the SSOT registry" in the "How the LSP shares the compiler" section)
...
3. Draft [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) schema reference. Required entry-kinds: `keyword`, `banned_jargon`, `primitive_intrinsic`, `type_attached_constant`, `deferred_language_feature`, `deferred_tooling_feature`, `deferred_stdlib_api` (RESERVED, unpopulated), `diagnostic_template`, `muted_hint_domain`. Each entry-kind documented with required fields and example TOML.
...
6. Draft [`.claude/rules/feature-registry.md`](../../../rules/feature-registry.md): when to load this rule (any milestone touching the registry OR adding a language feature OR renaming an internal method), required-entry-types checklist.
7. Draft [`.claude/rules/plan-invariants.md`](../../../rules/plan-invariants.md) `### Feature Registry Entries` subsection. Required content: enumerate every registry entry the milestone adds; for renames/changes, enumerate every registry entry the milestone modifies. Marked "applies to plans from v0.2-M2 onward" (this plan is exempt).
8. Draft [`.claude/graveyard.md`](../../../graveyard.md) entry. Pattern name: "scattered-registry-without-SSOT". Grep pattern: detects new `pub static FOO: &[&str] = &[...]` (or `const`, or `[(name, replacement)]`-style tuple arrays) within `crates/ynz-{diagnostics,typeck,parser}/src/` that doesn't have an explicit `// CARVE-OUT: SSOT registry/features.toml#<entry>` comment within 3 lines above. Test the pattern locally against the current code (should match the things we're about to migrate; should NOT match other arrays in those crates).
9. Update [`CLAUDE.md`](../../../../CLAUDE.md) Project Layout + Rules Files table to point at the new rule + design doc.
10. Update [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) with TBD placeholder lines for v2+ deferred entries (each gets `Registry entry: TBD M1 (P5b)`).
11. Update [`docs/internal/decisions/ADR-compiler-language.md`](../../../../docs/internal/decisions/ADR-compiler-language.md) with the SSOT registry paragraph.
12. Audit `.claude/plans/active/m7-*.md` / `m8-*.md` per roadmap Round-1 flag; log to `todos.md` if action needed.

**Acceptance criteria**:
- [x] [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) exists with every required section (schema reference / consumer API / Carve-Outs / self-hosting migration / when-to-add / when-not-to-add)
- [x] [`.claude/rules/feature-registry.md`](../../../rules/feature-registry.md) exists with load-trigger + required-entry-types checklist
- [x] [`.claude/rules/plan-invariants.md`](../../../rules/plan-invariants.md) has `### Feature Registry Entries` subsection clearly marked "applies to plans from v0.2-M2 onward"
- [x] [`.claude/graveyard.md`](../../../graveyard.md) has the scattered-registry entry with a tested grep pattern (the grep pattern matches every current scattered registry that's about to migrate, and zero false positives elsewhere in the crates)
- [x] [`CLAUDE.md`](../../../../CLAUDE.md) Project Layout + Rules Files tables reference the new doc + rule
- [x] [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) has TBD placeholder for each v2+ deferred entry
- [x] [`docs/internal/decisions/ADR-compiler-language.md`](../../../../docs/internal/decisions/ADR-compiler-language.md) has the SSOT registry paragraph
- [x] m7/m8 plan-file audit logged to `todos.md` (action or no-action)

**Quality gate**:
- [x] Schema example TOML in [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) is parseable by the `toml` crate — verified via `python3 tomllib` during Phase 1 code-review; all 8 entry-kind example blocks parsed cleanly
...
- [x] No banned-jargon words introduced in the new docs (three `infer` hits in docs/internal/implementation/IMP-feature-registry.md are engineer-audience prose — dual-audience exemption per [`.claude/rules/inference.md`](../../../rules/inference.md))

**Verification**:
- `cd /workspaces/ynz && cargo build --workspace` (sanity — should be unchanged, no code touched)
- Manual read-through of [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) from the perspective of "I'm a new contributor adding a new keyword — does this doc tell me what file to edit and what fields are required?"
...
**Est. lines**: ~500
**Ships via**: `/pr`
**Objective**: Build the foundation that Phases 2-7 plug into. After this phase, `cargo build` consumes [`registry/features.toml`](../../../../registry/features.toml) and generates `OUT_DIR/registry.rs`; the `ynz-registry` crate exposes typed accessors; sketch consumers in tests prove the schema is workable.
...
**Current-state anchors**:
- [`Cargo.toml`](../../../../Cargo.toml) (workspace) — current `members = [...]` list
...
**Files (expected scope)**:
- NEW [`crates/ynz-registry/Cargo.toml`](../../../../crates/ynz-registry/Cargo.toml) (build-dep on `toml`, runtime-dep on nothing)
...
- NEW [`registry/features.toml`](../../../../registry/features.toml) (~30 lines: empty entry per kind as schema-validation placeholder — `[[keyword]] name = "PLACEHOLDER_FOR_PHASE_4"` style, deleted in migration phases as they're filled in)
- UPDATE root [`Cargo.toml`](../../../../Cargo.toml) — add `crates/ynz-registry` to workspace members
...
2. Implement [`crates/ynz-registry/Cargo.toml`](../../../../crates/ynz-registry/Cargo.toml) with `[build-dependencies] toml = "..."` (look up current stable version via Context7 / crates.io). Runtime deps: none.
3. Implement `build.rs`:
   - Read `OUT_DIR` and `CARGO_MANIFEST_DIR` env vars
   - Parse `../../registry/features.toml` (relative to crate root)
   - Validate each entry against the schema from [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) (panic with clear error if a required field is missing — message identifies the file, the entry kind, the entry name, and the missing field)
...
7. Populate [`registry/features.toml`](../../../../registry/features.toml) with ONE placeholder entry per kind (validates schema, gives sketch consumers something to find).
...
- [x] `build.rs` declares `cargo:rerun-if-changed` correctly — touching `ynz-typeck/src/lib.rs` does NOT re-run build script; touching [`registry/features.toml`](../../../../registry/features.toml) DOES re-run
...
**Quality gate**:
- [x] Schema in [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) matches what `build.rs` accepts (re-read both; all 9 entry kinds match)
...
- Touch [`registry/features.toml`](../../../../registry/features.toml) and rebuild — confirm `ynz-registry` build script DOES re-run
...
**PR scope**: Move all 30 banned-jargon entries + 8 acronyms from `crates/ynz-diagnostics/src/banned_jargon.rs` to [`registry/features.toml`](../../../../registry/features.toml). The `.rs` file becomes a thin adapter that reads from the registry crate.
...
**Files (expected scope)**:
- UPDATE [`registry/features.toml`](../../../../registry/features.toml) — add 30 `[[banned_jargon]]` entries + 8 acronym entries (each: `name`, `replacement`, `reason`)
...
- UPDATE [`crates/ynz-diagnostics/Cargo.toml`](../../../../crates/ynz-diagnostics/Cargo.toml) — add `ynz-registry` dep
- DELETE the placeholder `[[banned_jargon]]` entry that Phase 1 put in [`registry/features.toml`](../../../../registry/features.toml)
...
2. For the 8 acronym entries (UTF-16, etc.) — likely a separate `[[banned_jargon_acronym]]` kind OR a `kind = "acronym"` field on the same `[[banned_jargon]]` entry. Decide based on what reads cleanest in [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) schema.
...
**Acceptance criteria**:
- [x] All 30 banned-jargon entries + 8 acronyms appear in [`registry/features.toml`](../../../../registry/features.toml) with identical text to pre-migration
...
**Files (expected scope)**:
- UPDATE [`registry/features.toml`](../../../../registry/features.toml) — add `[[primitive_intrinsic]]` entries (each free_fn + method + methods_1arg + print_type) and `[[type_attached_constant]]` entries (int.max/min, float.max/min/epsilon, number.max/min/epsilon)
...
- UPDATE [`crates/ynz-typeck/Cargo.toml`](../../../../crates/ynz-typeck/Cargo.toml) — add `ynz-registry` dep
...
- UPDATE [`crates/ynz-codegen/Cargo.toml`](../../../../crates/ynz-codegen/Cargo.toml) if needed
...
**Acceptance criteria**:
- [x] All `PrimitiveIntrinsicTable` entries appear in [`registry/features.toml`](../../../../registry/features.toml)
- [x] All 7 type-attached-constant entries appear in [`registry/features.toml`](../../../../registry/features.toml)
...
- UPDATE [`crates/ynz-parser/Cargo.toml`](../../../../crates/ynz-parser/Cargo.toml) — add `ynz-registry` dep
...
2. Add entries to [`registry/features.toml`](../../../../registry/features.toml).
...
**Acceptance criteria**:
- [x] All ~50 keywords + ~20 banned-declaration-keywords appear in [`registry/features.toml`](../../../../registry/features.toml)
...
**Files (expected scope)**:
- UPDATE [`registry/features.toml`](../../../../registry/features.toml) — add `[[deferred_language_feature]]` entries for: `f32`, `f64`, `i8`, `i16`, `i32`, `i64`, `u8`, `u16`, `u32`, `u64` (each with `substitute = "int"` or `substitute = "float"` or `substitute = "number"`, `why = "..."`, `ships_in = "v2+"`, `design_doc = "docs/reference/REF-mvp-scope.md"`) and `test` (`substitute = ""` since there's no current substitute, `why = "..."`, `ships_in = "v0.12"`, `design_doc = "docs/reference/REF-mvp-scope.md"`)
...
**Acceptance criteria**:
- [x] All sized-int / sized-float / `test` entries appear in [`registry/features.toml`](../../../../registry/features.toml) as `[[deferred_language_feature]]`
...
**Why this phase exists**: The roadmap explicitly enumerates this work: "Reserved-but-deferred features (every entry in [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) v2+ section AND every entry in `design/future/*.md`: arena, auto-soa, concurrency, http-framework, no-runtime-mode, packages, panic-safety, self-references, supervisor) → registry entries with locked-design pointers." Plus the `release-mode.md` and `string-ptr-len-overhaul.md` and `inline-shape-types.md` files added more recently.
...
- [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) — v2+ section (placeholder lines added in Phase 0)
...
- UPDATE [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) — back-fill the placeholder lines from Phase 0 with the actual registry-entry names (`Registry entry: deferred_language_feature.arena`, etc.)
- UPDATE `crates/ynz-registry/tests/` — add a `tests/design_future_sync.rs` test that walks `design/future/*.md` filenames and asserts every file (except a hardcoded skip list — currently exactly `index.md` — declared as `const SKIP: &[&str] = &["index.md"];` at the top of the test file with a comment naming the rationale) has a corresponding registry entry, and every registry entry's `design_doc` field points to an existing file. Adding a new index-style file (e.g., [`README.md`](../../../../README.md)) requires editing `SKIP` — explicit, not implicit. Avoids the false-positive drift of allowlist-by-pattern.
...
3. Backfill [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) placeholder lines.
...
- [x] [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) placeholder lines from Phase 0 are filled in with real registry-entry names
...
**PR scope**: Add `[[diagnostic_template]]` entries for the canonical / reusable diagnostics (e.g., "cannot mutate const", "use after give", "hidden field access from outside file"). Add `[[muted_hint_domain]]` entries for every domain in [`.claude/rules/inference.md`](../../../rules/inference.md). NO consumer wiring — these entries are populated for v0.2-M2 LSP to consume.
...
- [`.claude/rules/inference.md`](../../../rules/inference.md) — muted-hint domain catalog (type inference, function param type inference, ownership at call sites, wait points, lifetimes, allocators, copy points, `array<T>`→`fixed<T>` promotion, `let`→`const` promotion)

**Files (expected scope)**:
- UPDATE [`registry/features.toml`](../../../../registry/features.toml) — add `[[diagnostic_template]]` entries for each `DiagnosticKind` variant (each: `kind_name`, `what_template`, `what_instead_template`, `why_template` — these are PARAMETRIZED with `{placeholders}` since the dynamic message-construction stays in code; the registry just owns the canonical text shape)
- UPDATE [`registry/features.toml`](../../../../registry/features.toml) — add `[[muted_hint_domain]]` entries (each: `domain`, `placement_category` ∈ {`Addition`, `Replacement`, `Informational`}, `description`, `example_source`, `example_hint_rendered`)
...
2. Inventory muted-hint domains from [`.claude/rules/inference.md`](../../../rules/inference.md). For each domain row in the table, draft a TOML entry with example source code + example rendered hint text (copy from the rules file).
...
- [x] Every muted-hint domain from [`.claude/rules/inference.md`](../../../rules/inference.md) has a `[[muted_hint_domain]]` entry with `placement_category` set correctly
- [x] `cargo test --workspace` passes
...
**Verification**:
- `cd /workspaces/ynz && cargo test --workspace`
- Manual TOML inspection: every domain table row from [`.claude/rules/inference.md`](../../../rules/inference.md) appears as an entry
...
**Current-state anchors**:
- [`.claude/graveyard.md`](../../../graveyard.md) — entry from Phase 0 with grep pattern
...
- UPDATE [`.claude/graveyard.md`](../../../graveyard.md) — add a test-the-Bouncer-pattern verification note: "Pattern tested against current main on YYYY-MM-DD; matches the X scattered registries about to migrate; zero false positives in other crates."
...
**PR scope**: Full-system verification phase. Extend examples/primantis-orders with the new error gallery file, sanity-check examples/pirates-roster still runs unchanged (it shouldn't — this is a refactor), bump [`Cargo.toml`](../../../../Cargo.toml) to `0.2.0-m1`, commit, tag, optionally publish (but Yinz isn't on crates.io yet — skip).
...
- [`Cargo.toml`](../../../../Cargo.toml) (workspace + per-crate) — current version `0.1.0`
...
- UPDATE [`Cargo.toml`](../../../../Cargo.toml) (workspace + all crates) — bump version to `0.2.0-m1`
- UPDATE [`CHANGELOG.md`](../../../../CHANGELOG.md) — generate section from merged PRs since `v0.1.0` per `/release` skill
- UPDATE [`.claude/state.md`](../../../state.md) — append M1-complete decision row (with WHY); status: M1 SHIPPED
- UPDATE [`.claude/todos.md`](../../../todos.md) — mark v0.2-M1 done in Done section
...
- [x] [`Cargo.toml`](../../../../Cargo.toml) bumped to `0.2.0-m1`
- [x] [`CHANGELOG.md`](../../../../CHANGELOG.md) section added covering M1
...
|---|---|---|
| `[[stdlib_api]]` entry-kind reserved in schema, populated with ZERO entries in M1 | [`docs/internal/implementation/IMP-feature-registry.md`](../../../../docs/internal/implementation/IMP-feature-registry.md) schema reference (created Phase 0) + this plan's Constraints section | Each v0.5+ stdlib milestone (file, math, cli, json, etc.) populates its own entries — bound to the milestone delivering the API |
| `### Feature Registry Entries` 7th invariants subsection — this plan is exempt | [`.claude/rules/plan-invariants.md`](../../../rules/plan-invariants.md) 7th subsection text (created Phase 0) marked "applies to plans from v0.2-M2 onward" | Every v0.2-M2-onward plan must include the subsection; rule mechanically enforces |
...
| [`docs/reference/REF-mvp-scope.md`](../../../../docs/reference/REF-mvp-scope.md) placeholder lines created in Phase 0 with `Registry entry: TBD M1 (P5b)` | This plan's Phase 0 (creates placeholder) + Phase 5b (back-fills with real entry names) | Phase 5b acceptance criterion includes back-fill verification |
| CI wiring of `cargo test -p ynz-registry` (Phase 7) may defer to a separate chore PR if CI doesn't exist yet | [`.claude/todos.md`](../../../todos.md) Later section: "Wire up GitHub Actions CI (ci.yml already written, just needs configuration)" already tracks the broader CI deferral | When the broader CI wiring lands, this test gets added to the CI step list. Phase 7 PR description names the test for the future CI PR to wire. |
