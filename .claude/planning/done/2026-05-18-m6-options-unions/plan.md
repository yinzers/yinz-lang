---
name: "m6-options-unions"
plan-id: "2026-05-18-m6-options-unions"
status: "done"
...
**examples/primantis-orders/m6_errors.ynz** — create in P6, intentionally trigger:
...
Each error in the file has a `// WHY:` comment naming the diagnostic class. The file is run via the existing `examples/primantis-orders/m{N}_errors.ynz` snapshot harness in `crates/ynz-driver/tests`.
...
- `examples/primantis-orders/m6_errors.ynz` — CREATE in this phase; add the options-related error classes here (non-exhaustive options multi-case; comparison-between-options; ambiguous-shorthand; function-vs-shorthand collision; single-variant-options rejection; 256+-variant options rejection). Per the `### Demo & Error Gallery` per-phase obligation.
- `examples/pirates-roster/entrypoint.ynz` — extend with M6 options demo (declare `options Status { pending, active, banned }`; print `Status.active.toString()` deferred to P4 codegen; for P3a, declare + use via assignment + comparison). Per the per-phase obligation.
...
10. **Add to `examples/primantis-orders/m6_errors.ynz`** (create file): the options-related error cases listed in Files (each with `// WHY:` comment).
11. **Add to `examples/pirates-roster/entrypoint.ynz`**: M6 options section showing declaration + value use + comparison + multi-case dispatch. Final `.toString()` line marked `// after P4 codegen lands`.
**Acceptance criteria**:
...
- [ ] `examples/primantis-orders/m6_errors.ynz` created with options error cases (and grows in P3b/P4 for union/narrowing cases).
- [ ] `examples/pirates-roster/entrypoint.ynz` extended with M6 options section.
...
**Verification**: `cargo test -p ynz-typeck` shows ~20 new tests; `./target/debug/ynz run examples/primantis-orders/m6_errors.ynz` produces all options-related error classes; `./target/debug/ynz run examples/pirates-roster/entrypoint.ynz` runs through the new M6 options section.
...
- `examples/primantis-orders/m6_errors.ynz` — extend with union/narrowing error cases (this phase adds: non-exhaustive union multi-case; `is Type` on non-union; `is Type` with type not in union; `.foo` access without `is`; union `Foo | Foo`; non-adjacent duplicate `Foo | Bar | Foo`; single-variant union; namespace shadow for `is X`; `||` non-propagation diagnostic; reassignment invalidates; `lend` call invalidates; non-recognized-exit form for early-return; redundant `is Foo` INFO diagnostic; nested-narrowing edge cases)
- `examples/pirates-roster/entrypoint.ynz` — extend with M6 union + narrowing demo (classify-account union; early-return narrowing example; `.toInt()` runtime conversion final line — codegen lands in P4 but typeck approves here)
...
15. **Extend `examples/primantis-orders/m6_errors.ynz`** with all union + narrowing cases per Files list above (each with `// WHY:` comment).
...
**Verification**: `cargo test -p ynz-typeck` shows new tests; spot-test `./target/debug/ynz run examples/primantis-orders/m6_errors.ynz` shows all union+narrowing error classes; the locked-text diagnostics produce byte-identical output to the spec.
...
- [ ] **Per-phase Demo + Gallery obligation**: `examples/pirates-roster/entrypoint.ynz` M6 section runs end-to-end after P4 (the `.toString()` and runtime conversion lines from P3a/P3b that were marked `// runs after P4 codegen` now run). `examples/primantis-orders/m6_errors.ynz` extended with any P4-specific error cases (e.g., if any new errors emerge during codegen — typically just confirmation that P3-time errors still fire end-to-end).
...
**Steps**:
1. Extend `examples/pirates-roster/entrypoint.ynz` per the demo content listed in `### Demo & Error Gallery`. Run it; assert stdout matches expectations.
...
**Acceptance criteria**:
- [x] `examples/pirates-roster/entrypoint.ynz` runs end-to-end through every milestone (M1-M6) demonstrated.
