...
- **Hosting = Cloudflare Pages** (free tier for OSS). Locked. — Rejected: Vercel/Netlify (trial-cliff pricing risk for OSS), GitHub Pages (no Nuxt SSR/edge story if we ever need it), self-host (operational overhead unjustified for a docs site).
...
| Hosting provider price shock or feature regression mid-project | Low | Medium — could force migration | Pick hosting with multiple viable alternatives (Cloudflare Pages → can migrate to Netlify or self-host on Coolify with minimal config change); avoid platform-lock-in like Vercel proprietary edge functions |
...
1. **Hosting platform** — ANSWERED: Cloudflare Pages (free tier for OSS, locked).
