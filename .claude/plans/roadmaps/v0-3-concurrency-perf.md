...
### Milestone 2: v0.3-M2 — `wait` Suspension + State Machine Codegen — multi-session
**Value delivered**: Functions with explicit `wait` calls actually suspend and resume instead of blocking. An I/O-bound function that calls `wait db.fetch(query)` no longer blocks its OS thread — it yields to the Tokio scheduler, allowing other tasks to run while waiting for the I/O. This makes existing `wait` code correct (not just syntactically valid). The full state machine transformation for multi-`wait` functions ships here.
...
**Rough scope**:
- M2 P0: research spike — prototype the simplest case (`function fetchOne() -> string errors { let x = wait db.fetch("q") }`) as a hand-written LLVM IR state machine; validate Tokio polling protocol integration end-to-end
- New `lower_function_with_waits()` codegen path in `emit.rs`; transforms a function containing `Expr::Wait` into an LLVM IR state machine struct + `resume(ctx, waker)` function
- State machine frame allocation: heap-allocated (via `ynz_alloc`) on spawn; freed by Tokio task completion
- `Expr::Wait(inner, _)` desugars to: call `inner` → if result is ready, continue; if pending, register waker + return `Poll::Pending` to Tokio
- `wait` at a non-may-block call site: compile warning (waiting for something that can't block; `wait` is unnecessary)
- Integration tests: function with one `wait`, function with two sequential `waits`, function with `wait` inside `if`, nested function calls with `wait`
- **Note**: "may-block" analysis (which calls CAN block) is M3. M2 only makes EXPLICIT user-written `wait` work correctly.
...
### Milestone 3: v0.3-M3 — May-Block Analysis + Auto-Parallelization — multi-session
**Value delivered**: Independent operations in a function body auto-parallelize without the user writing `background` or `wait`. The compiler's dependency graph analysis identifies statement groups with no shared mutable state and schedules them concurrently. The compiler also inserts `wait` at I/O call sites automatically — users don't need to write `wait db.fetch(q)` if the compiler proves `db.fetch` may block. The IDE's muted `wait` hint surface (per `design/ide-hints.md` `wait_points` domain, currently protocol-only) starts firing.
...
**Rough scope**:
- Whole-program may-block analysis: new salsa-tracked `may_block_query(db, source) -> HashMap<String, bool>` pass. Fixed-point propagation: I/O intrinsics start as `may_block = true`; user functions inherit from callee graph. Cross-module propagation via M8's compile-time cross-file typeck (M8 ships before M3 P2 — decided 2026-05-21).
...
- **Teaching surface (REQUIRED in M4, not deferred)**:
  ...
  - New `[[lint_rule]]` registry entries: `array-using-soa-layout` + `cross-thread-fields-not-padded` — each with WHAT/WHAT-INSTEAD/WHY hover text
