...
- **No new language features in v0.2** — this milestone is tooling. Auto-promotion analyses, lint rules, and stdlib stay deferred to their assigned versions (v0.3, v0.4, v0.6+).
...
- **Does the SSOT registry track stdlib APIs that don't exist yet (v0.6+ placeholders), or only language-level features?** Tracking stdlib placeholders means v0.2-M1 has to predict the shape of every v0.6+ module entry — premature. NOT tracking them means v0.6+ has to retrofit the registry. Compromise: v0.2-M1 designs the registry SCHEMA to accommodate stdlib API entries, but only POPULATES it with v0.1 language-level entries + the deferred-tooling-feature entry-type (so v0.2-M4 can register `ynz watch --json` if punted). Each v0.6+ stdlib milestone POPULATES its own entries. Blocks: v0.2-M1 scope. Confirm in v0.2-M1 execution plan.
...
- New deferred-feature trigger entries in `examples/errors/m9_errors.ynz` if the registry rewrites any deferred-feature error rendering (likely yes — `gpu`, `test`, `foreign`, `int<32>`, `f32`, `number<5000>` all get registry-driven errors now).
...
- `examples/basics/src/entrypoint.ynz` annotated comments showing "open this in VSCode with the Yinz extension installed to see X"
...
- **Package manager** — v0.5.
- **Any stdlib module** — v0.6+ (file/path/directory), v0.7 (math), v0.8 (cli/env/process), v0.9 (json), etc. The SSOT registry schema should accommodate stdlib API entries (confirmed in open question above), but v0.2 only populates language-level entries.
...
| Formatter idempotency bug ships — `fmt(fmt(x)) != fmt(x)` — breaks CI for users using `--check` | Low | High | v0.2-M3 has explicit `fmt(fmt(x)) == fmt(x)` property test running across all spec/ examples + all `examples/basics` content. Fuzz test (`proptest`) over arbitrary AST inputs as the deeper guarantee. |
