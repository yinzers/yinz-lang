...
| **8** | **M11** (content polish) | Large (9 phases) | Standalone — ships as `v0.2.1.1` content patch AFTER the main tag (Q13). Pure content + small hover additions. | after release tag |
...
**Value delivered**: the 96 primitive intrinsic entries that ship with empty hover docs get populated. The 9 pedagogically critical keywords (`follows`, `extends`, `base`, `dynamic`, `hidden`, `wait`, `background`, `errors`, `sensitive`) get hover content beyond "Introduced in M4". Per-domain hover WHY templates replace the generic `placement_category` fallback that makes all 5 Addition-category domains share one WHY paragraph. Inlay hint render strings get concrete-type + binding-name context (e.g., `// promoted to fixed<int, 3> — nums never grown after declaration` instead of `// promoted to fixed — never grown`). The 15 highest-leverage diagnostic WHY-quality improvements identified by the diagnostic-quality audit ship. Shape and options declarations get hover content showing field listings. Field hover on dot-access (`p.health` cursor on `health`) returns the field type.
**Execution plan**: `v0-2-1-m11-teaching-content-polish` (status: planned)
**Depends on**: nothing — pure content + small typeck additions for hover field lookup.
...
- **P8 — Shape/options hover + field hover on dot-access**: extend `hover_response` to look up shape names + options names in `shape_table` and emit a hover body listing fields/variants (calling existing `doc_for_name` for `Item::ShapeDecl` / `Item::OptionsDecl`). Add field-access detection: when the cursor token resolves to neither a keyword nor a function, check whether the byte offset falls inside a `FieldAccess` expression; if so, look up the receiver's type from `expr_types` and resolve the field type from `shape_table`. Emit `field_name: FieldType` plus the field's `///` doc comment if present.
