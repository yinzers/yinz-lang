...
4. Write `design/future/http-framework.md`: supervision-by-default. `server.listen()` returns a supervised server. Request handlers run in isolated `background` tasks. Accept loop is supervised — if it crashes, framework restarts it. Default 500 handler. Custom panic policy via `supervise:` config option. IDE shows muted supervision config. v0.3+ (after stdlib basics in v0.2). Reference the BullMQ analogy from the conversation.
