...
**Acceptance criteria**:
- [ ] All three functions exported with `#[no_mangle]` C ABI + doc comments covering Flow/Failure modes/Side effects/Safety (house pattern in runtime.rs)
  - Evidence: (filled at phase completion)
- [ ] Runtime unit tests cover: value roundtrip per payload class, saturation, drop-detach with alloc accounting, panic re-raise, pre-init/post-shutdown discard
  - Evidence: (filled at phase completion)
- [ ] `runtime_decls.rs` declares all three for codegen consumption
  - Evidence: (filled at phase completion)
- [ ] Zero behavior change to existing exports (full workspace suite green)
  - Evidence: (filled at phase completion)
**Quality gate**:
- [ ] No `block_on`/park anywhere in the new code
- [ ] Waker forwarding matches the sleep-poll discipline (no fabricated wakers)
- [ ] Ownership of ctx/handle/result documented at every transfer point
- [ ] Follows existing runtime.rs doc-comment + error-path conventions
