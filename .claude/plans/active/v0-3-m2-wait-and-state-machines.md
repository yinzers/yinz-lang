...
- [ ] `-> string errors` SM: error HANDLED via `.or()` reads the SUCCESS slot; AND error-through-suspension surfaces with the EXACT expected message/trace value computed from the fixture (`assert_eq!`, not "surfaces correctly") + `{i64,i64}` field order asserted (error_ptr=field0, success=field1)
  - Evidence: (filled at phase completion)
- [ ] A function that is `suspends=true` but `contains_wait=false` (transitive, no explicit `wait` — e.g., `bar` calls `sleepAsync` without wait, `entrypoint` calls `bar` without wait) RUNS correctly: prints expected output, exits 0, no panic. This is the P6/P7 seam case that was unsafe to fix in P6 (P7 inline-poll-yield is required).
  - Evidence: (filled at phase completion)
