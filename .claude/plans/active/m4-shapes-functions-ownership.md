...
|---|---|---|---|
| **P1** (Lexer) | `feat/m4-lexer` | ✅ MERGED (05c5296) | 8 new tokens (49→57), 6 banned-keyword handlers, 63 lex + 49 parse tests green. |
| **P2** (Parser) | `feat/m4-parser` | ✅ MERGED (84db1d2) | ShapeDecl AST, FieldAccess, StructLit, PostfixOp, SelfValue, FieldAssign, Dynamic/SelfType, ownership modifiers. 68 parse tests green. |
...
| **P5** (Catch-up) | `feat/m4-catchup-numerics` | ✅ IN REVIEW | 6 wrapping/saturating int methods (LLVM wrapping add/sub/mul + sadd.sat/ssub.sat/smul.fix.sat), `int.max`/`int.min` type-attached constants, `number.epsilon`/`max`/`min`, `float.max`/`min`/`epsilon`. M2 catch-up fixtures created and passing. All tests green, clippy clean. |
