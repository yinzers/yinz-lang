...
**Files (expected scope)**: `crates/ynz-typeck/src/may_block.rs`, `crates/ynz-typeck/src/queries.rs`, `crates/ynz-typeck/src/check.rs`, fixtures under `crates/ynz-driver/tests/fixtures/`
**Deviation rule**: standard.
**Steps**:
1. Extend `may_block::analyze` to accept the imported functions' `suspends` flags (a `HashSet<String>` of imported names known to suspend, derived from `SignatureOutput.imported_fns`). Seed the fixpoint with local functions that call an imported-suspending function (in addition to the intrinsic-callers seed).
2. In `check_query` (`queries.rs:171-174`): stop overwriting imported `suspends`. Compute the union of (a) imported fns' own `suspends` (preserved) and (b) local fns the extended fixpoint now marks. Set each merged sig's `suspends` from that union.
3. Remove the cross-module stopgap (`check.rs:2383-2400`). A cross-module call from a suspending function is now resolved (the callee's `suspends` is known from `imported_fns`).
4. Verify codegen reads the cross-module-derived `suspends` (it already consumes `check.suspends_set` per `codegen/queries.rs:39-51`) — a cross-module suspending caller must get `lower_function_with_waits`.
5. Add fixtures: (a) module A `function slow() { wait sleep(50) }`, module B `import slow; function caller() { slow() }` → compiles + runs; (b) the OLD stopgap repro now compiles; (c) a non-suspending cross-module call stays non-SM (no over-marking).
6. Add incremental-correctness tests (salsa), all three directions: (a) A's `slow` non-susp→susp re-checks B and flips B's caller to a state machine; (b) the INVERSE — susp→non-susp re-checks B and B's caller STOPS being a state machine (over-marking clears, no stale `suspends`); (c) the diamond — A imported by both B and C; toggling A re-checks both. These are where salsa staleness hides.
**Acceptance criteria**:
- [ ] A cross-module call to a suspending function compiles and runs correctly (live `ynz run`, correct stdout)
  - Evidence: (filled at phase completion)
- [ ] The `check.rs:2383` cross-module stopgap error is removed; its prior repro fixture now compiles + runs
  - Evidence: (filled at phase completion)
- [ ] A non-suspending cross-module call does NOT mark its caller as a state machine (no over-approximation) — verified by IR/`suspends` inspection or a fixture that would mis-suspend
  - Evidence: (filled at phase completion)
- [ ] Incremental tests (salsa) pass in all three directions: non-susp→susp flips B's caller to SM; susp→non-susp clears it (no stale over-marking); diamond (A→B, A→C) re-checks both
  - Evidence: (filled at phase completion)
- [ ] `cargo test --workspace` green (no regression on the existing `v0_3_m2_cant_infer_cross_module` family — those fixtures/snapshots updated to the now-compiling behavior)
  - Evidence: (filled at phase completion)
**Quality gate**:
- [ ] Conservative-correct: unresolved edges (dynamic dispatch, FFI) still produce a clean error, not silent non-suspension
- [ ] No `as any`-class escape hatches; no test weakening (immutable-test-check)
- [ ] Salsa dependency on imported signatures is real (incremental test proves it)
