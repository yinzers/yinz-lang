...
- DNS resolution has known limitations (musl doesn't support `/etc/nsswitch.conf` plugins, doesn't fall back to TCP for large responses on older versions). **Mitigated** by Yinz's network stdlib using its own resolver where appropriate (decision deferred to v0.15 http milestone).
