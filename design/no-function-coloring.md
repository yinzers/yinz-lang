...
The muted `64` appears in the position where the user would have typed it, not as a comment after the line. Click → "64" gets typed into the source. This is the addition-placement pattern from `.claude/rules/inference.md`.
...
- `.claude/rules/stdlib-design.md` Rule 4 (bounded by default) — channel/queue is the canonical instance of this rule.
...
- Values already run cleanup via drop-on-unwind (per `design/future/panic-safety.md`)
