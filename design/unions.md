...
See `spec/unions.md` for the user-facing surface.
...
| `T \| none` (any T) | NOT a union per M6 codegen — this IS `maybe<T>` and uses M5's lowering table | Avoids double-encoding; the `T | none` form at typeck time is rewritten to `maybe<T>`. See `design/maybe.md`. |
...
- `spec/unions.md` — user-facing surface (how to write unions and use `is`)
- `design/options.md` — options types (the label-only counterpart)
- `design/narrowing.md` — flow-sensitive `is` narrowing (how the compiler tracks which variant is proven inside a block)
- `design/maybe.md` — `maybe<T>` lowering (the `T | none` special case; unions reuse M5's maybe encoding for that form)
- `design/control-flow.md` — multi-case `if` exhaustiveness; jump-table lowering
- `design/golden-rules.md` Rule 6 — borrow familiar syntax (`|` from TypeScript)
