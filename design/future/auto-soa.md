...
4. **No external code requires the AoS layout** (no FFI export, no serialization that depends on byte layout — the serialization codegen per `design/stdlib-design.md` Rule 6 produces wire format independent of internal layout, so this is usually automatic).
...
The SoA analysis pass scales linearly with code size. Realistic estimate: +3-7% on `ynz build --release` for code bases with many `array<shape>` hot loops. Negligible on `ynz build` (debug) because the optimization pass is skipped in debug builds (debug-vs-release cost separation per `design/compiler.md`).
...
The IDE surface for auto-SoA is **NOT a muted-hint inference** (the protocol in `.claude/rules/inference.md` requires every muted hint to complete to typeable Yinz syntax — SoA has no opt-in syntax for the user to type, so the muted-hint protocol doesn't apply).

Instead, auto-SoA uses the same hybrid model as `array<T>` → `fixed<T>` promotion (`design/collections.md`): silent codegen + Tier 3 lint suggestion. The transform happens; the IDE surfaces a Tier 3 suggestion (yellow squiggle) on the array declaration that explains the optimization in hover.
...
- **WHAT INSTEAD**: There is no source-level opt-in or opt-out syntax for SoA in v0.3. The compiler picks per-array based on access patterns. If you need a specific layout for FFI or a wire format, FFI bindings (v2+) and the serializer codegen (per `design/collections.md` "Auto-promotion" section) handle the conversion at the boundary, not at the source.
...
- `design/golden-rules.md` Rule 4 (compiler does the hard work)
- `design/golden-rules.md` Rule 8 (zero-cost abstractions — high-level syntax compiles to optimal layout)
- `design/golden-rules.md` Rule 10 (efficiency first — fast layout is the default, no opt-in needed)
- `design/golden-rules.md` Rule 11 (compiler is teacher — IDE hint explains the transform)
- `design/ownership.md` (the aliasing proof system that makes auto-SoA possible)
- `design/concurrency.md` (auto-parallelization — same v0.3 milestone family)
- `design/collections.md` (`array<T>` and `shape` semantics this builds on)
- `.claude/rules/inference.md` (the muted-hint protocol the IDE surface uses)
