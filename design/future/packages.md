...
User spec target: `spec/packages.md` (already exists for source-level package model; this adds the BINARY format spec).
...
- **Not the source-level package model** — that's in `spec/packages.md` and `design/packages.md`. Those documents are about `ynz add`, `ynz_modules`, `yinz.toml`, etc.
- **Not the package registry** — registry hosting/auth/publishing is a separate v0.2+ design (no doc yet; will be in `design/registry.md` when it's time).
- **Not an ABI specification** — Yinz packages are compiled per-target-triple. ABI stability across versions isn't promised (per `design/versioning.md`'s pre-release delete policy). This is the BINARY metadata format, not a stable wire protocol.
