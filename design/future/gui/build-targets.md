...
- [`architecture.md`](architecture.md) — the shell + bridge model these targets compile
- [`capabilities.md`](capabilities.md) — the capability declarations that become per-platform permission metadata
- [`../../no-runtime-mode.md`](../../no-runtime-mode.md) — `--kernel` is a *different* "no OS" target; the WASM/web target here assumes a browser host, not bare metal
