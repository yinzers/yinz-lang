...
If pixel-perfect rendering ever becomes a hard requirement, this is the path — and it builds on `design/gpu.md`, not on this webview model. It is plausibly its own product-sized initiative, not a milestone of this one.
...
This is deliberately deferred: the package system is not yet built (`design/future/packages.md`). When it ships, nothing in this webview model prevents a community or first-party package from offering a native/pixel renderer as an opt-in alternative. We document the door; we don't build it now. The user-facing "you can bring your own renderer" note belongs in `spec/` only once GUI actually ships (per `.claude/rules/spec-writing.md`, spec covers shipped features only) — until then it lives here and in the packages doc.

---

## Cross-references

- [`capabilities.md`](capabilities.md) — the device-capability + permission layer on top of the bridge
- [`build-targets.md`](build-targets.md) — per-platform compile + packaging + the WASM web target
- [`../packages.md`](../packages.md) — the package system that enables alternative-renderer packages
- [`../../gpu.md`](../../gpu.md) — the GPU foundation a future pixel renderer would build on
