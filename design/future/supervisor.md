...
supervise.alwaysRestart(processOrders)
  .maxRestarts(10, within: 1.minute)
  .onGiveUp((e) => alerting.page("processOrders died permanently", e))

// One-shot supervision: spawn the task, restart only if it panics (not on normal exit).
let handle = supervise.onPanic(processOrders, onPanic: (e) => log.error(e))
// handle.cancel() to stop supervising; handle.wait() to block until task exits

// Pool of supervised tasks (for parallel workers).
supervise.pool(processOrders, count: 8)
  .onPanic((worker, e) => log.error("worker ${worker.id} crashed", e))
```

The API is fluent chainable (this is one of the rare places fluent makes sense — supervisor config is a single declarative statement, not multiple steps). Names are dot-discoverable per Golden Rule 1.
...
### `.onPanic(callback)` / `.onGiveUp(callback)`
...
### `supervise.onPanic(task, onPanic)` — one-shot
