...
See `spec/maybe.md` for the user-facing surface.
...
- `.claude/rules/dot-postfix.md` — `.value` is access (no parens), `.exists()` and `.or()` are actions (parens)
- `.claude/rules/vocabulary.md` — `maybe T` (user) and `maybe<T>` (since M5 syntax-lock) terminology
