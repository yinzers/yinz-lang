...
### 1. Spec examples use double-quoted strings throughout — parser rejects them (HIGH)
...
### 3. spec/operators.md "Overloading" section describes a feature deferred to v1.0 (MEDIUM)
...
### 5. spec/iterables.md and parser handle `range(end)` 1-arg form but spec under-documents it (LOW)
...
### 6. spec/main.md mentions `process.exit(code)` and `cli.args()` — neither module exists in v0.1 (MEDIUM)
...
### 7. spec/sensitive.md uses method names that don't exist on `sensitive string` (HIGH)
