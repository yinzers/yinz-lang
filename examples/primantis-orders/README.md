...
| **v0_3_m5_errors.ynz** | deliberate omission — no entry | v0.3-M5 (by-value array/map storage + auto-SoA) shipped ZERO new compile-error classes. The contingent D6 loud-reject fallback never armed (the by-value parity gate stayed green), and M5 in fact REMOVED an error class: the `ArrayShapeRuntimeFieldWithWait` guard was LIFTED (its former triggers in `v0_3_m3a_errors.ynz` are annotated as lifted, not deleted history). Auto-SoA is a silent codegen layout choice surfaced via the Tier 3 lint `array-using-soa-layout` (a suggestion, not an error — no gallery trigger applies). Coverage lives in `crates/ynz-driver/tests/fixtures/m5_p3_*.ynz` / `m5_p4_*.ynz` and the pirates-roster demo instead. |
| **[v0_3_m7_errors.ynz](v0_3_m7_errors.ynz)** | ✅ shipped | Aliasing-call rejection (v0.3-M7 Phase 2, FRAGO 002): same value passed as both `share` and `lend`, same value passed to two `lend` positions, whole-vs-part overlap (`order` + `order.slip`). Closes the false-`noalias` optimizer miscompile class at compile time. |
