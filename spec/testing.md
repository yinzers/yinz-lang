...
Why the separation: if a single `assertFails` caught both errors AND panics, a typo in your test code that causes a panic would be silently counted as "yes it failed as expected" — and the test would pass for the wrong reason. With separate functions, panics from test bugs always propagate to the test runner with full stack trace. You see exactly where the test broke.
