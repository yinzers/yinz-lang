---
name: "IMP-strings"
description: "Locks Yinz's internal string encoding (UTF-8 always) and SIMD implementation strategy, plus UTF-8 defaults for file I/O, underlying the user-facing indexing API in docs/reference/REF-strings.md."
tags:
  - "yinz-compiler"
...
User spec: [`docs/reference/REF-strings.md`](../../reference/REF-strings.md) (indexing API, methods).

This file locks the internal-encoding decision and SIMD implementation strategy for the built-in `string` type. The user-facing API (`.byteAt()`, `.get()` by code point, `.graphemeAt()`, `.count()`/`.byteCount()`/`.graphemeCount()`) is documented in [`docs/internal/implementation/IMP-collections.md`](IMP-collections.md) "String Methods" section and [`docs/reference/REF-strings.md`](../../reference/REF-strings.md). This doc covers what's UNDERNEATH that API.
...
This extends [`.claude/rules/stdlib-design.md`](../../../.claude/rules/stdlib-design.md) Rule 3 (no silent platform-dependent defaults) to string case operations. The pattern: defaults protect the security-sensitive case; the i18n display case is opt-in with a visible parameter.

### Cross-references

- [`.claude/rules/stdlib-design.md`](../../../.claude/rules/stdlib-design.md) Rule 3 (no platform-default config)
...
- **The user-facing API** (methods like `.split()`, `.toUpperCase()`, etc.) — that's [`docs/reference/REF-strings.md`](../../reference/REF-strings.md) and the v0.5+ stdlib expansion.
- **Indexing semantics** (code-point vs byte vs grapheme) — covered in [`docs/internal/implementation/IMP-collections.md`](IMP-collections.md) "String Methods" section.
- **Locale-sensitive operations** (case conversion in Turkish locale, etc.) — open question for the stdlib design (see [`docs/internal/scratchpad/SCRATCH-stdlib-strings.md`](../scratchpad/SCRATCH-stdlib-strings.md) when written). Brief preview: locale-sensitive operations MUST take an explicit locale parameter; no system-locale-default for security-critical comparisons.
...
- [`docs/internal/implementation/IMP-collections.md`](IMP-collections.md) "String Methods" section (user-facing API)
- [`docs/reference/REF-strings.md`](../../reference/REF-strings.md) (user spec for string operations)
- [`docs/reference/REF-golden-rules.md`](../../reference/REF-golden-rules.md) Rule 4 (compiler does the hard work — SIMD selection)
- [`docs/reference/REF-golden-rules.md`](../../reference/REF-golden-rules.md) Rule 10 (efficiency first — UTF-8 is the efficient default for ASCII)
