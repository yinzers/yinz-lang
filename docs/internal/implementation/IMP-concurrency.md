...
- **The spawner's own reference is never released at scope exit.** No scope-exit drop-insertion pass exists in the compiler — no local of any kind is released at scope end (`ynz_handle_free` is declared in `crates/ynz-codegen/src/runtime_decls.rs:126` with zero emit sites; [`IMP-no-function-coloring.md`](IMP-no-function-coloring.md) "Task Cancellation" implementation note; the roadmap's never-drop-locals row); the drop ladder frees only the spawned tasks' references. So "all references except the consumer's are gone" is not an observable event today, even in principle.
...
### What is signed off, and what awaits Phase 2
...
**Awaiting Phase 2's sign-off:** the transfer rule itself ([`IMP-ownership.md`](IMP-ownership.md) "Transfer — Who Else Holds This Value"), and fr12's `number`-stays-copy-through decision with the `transfers_source()` split of `ChannelElemDrop`. The confirmation list is the Phase 2 sign-off packet in the v0.3-M8 plan's `audit.md`, not here.
