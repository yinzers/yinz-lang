...
## Channel Close — End-of-Stream Semantics (v0.3-M8; drafted through three review rounds, AWAITING Patrick's sign-off — the decisions still his are listed under "Open at sign-off" at the end of this section; ships Phase 4)

**Status**: designed in v0.3-M8 Phase 1 (2026-09-03), awaiting Patrick's sign-off. Nothing in this section is in the tree yet — v0.3-M8 Phase 4 implements it. Until then the shipped behavior is the one the "Bare `channel<T>` never closes" Design-Divergence entry below describes. This section is promoted out of that entry's one-paragraph format because the design has seven load-bearing parts (mechanism, name, receive typing, send ownership, idempotency, wake propagation, the deferred non-mechanism) that a divergence entry cannot carry. Two of those parts — `receive()` typed `maybe<T>` rather than `T errors`, and `send()` giving its payload — are rulings from the first review round, not the first draft (`git log --grep=m8-p1-fix1`). **The tree changed under this design after the second round**: the review's own use-after-free finding was hotfixed and merged ahead of M8 (`861fd4d`, merged at `ec014d8`) as a RUNTIME pointer-identity release protocol on the spawn-argument drop ladder. That protocol is now shipped substrate this design builds on, and "Two mechanisms, one rule" below states exactly how it and Phase 4's typeck consume divide the ownership question — read that subsection before the P2-3 guidance, because the closed-send free moved as a result (`git log --grep=m8-p1-fix2`). **The third round found that the give-at-send rule, as drafted, did not flow through an ordinary call** — a parameter sent by its function left the CALLER's binding live, so `wait producer(wire, rows)` followed by `rows.count()` was a deterministic use-after-free with no `background` anywhere. Patrick ruled for the guard: a parameter that is given away must be declared `give`, and a non-binding payload is refused. "Ownership must flow through the call" below carries the rule, its transit, and the two compile diagnostics it adds (`git log --grep=m8-p1-fix3`).
...
9. **`send()` gives its payload for owned-heap element types.** On a `channel<array<T>>` or `channel<map<K, V>>`, the binding passed to `send()` is consumed at the send; reading it afterward is a compile error. If that binding is a PARAMETER of the sending function, the parameter must be declared `give` — otherwise the send is a compile error — so the consume reaches every caller's binding through the existing `give` call-site path. A payload that is not a named binding, a `.copy()`, or a literal is refused. Primitive and `string` channels are unchanged. Full mechanism and reasoning in "`send()` gives its payload" and "Ownership must flow through the call" below.
...
**The rule.** `send()` transfers ownership of its payload. In `check_conduit_method_call`'s `send` arm (`check.rs:4105–4149`), when `channel_elem_drop(elem)` is `Some(..)` (an **owned-heap element**, below), the payload expression is classified into exactly three admitted forms and one refusal:

- **A plain identifier** — treated exactly as a `give` argument is in **`check_arg_ownership`** (`check.rs:4591–4648`; the user-function / UFCS parameter path, called from the plain call, monomorphized-generic call, and dot-call sites at `check.rs:4799`, `:5115`, `:5445`): a `const` binding is refused (the `:4602` refusal, **extracted** into a helper with a caller-supplied WHAT-INSTEAD, because it is an inline `format!` keyed on `fn_name`, and the send arm passes `Send a copy instead: {channel}.send({name}.copy()).` — a sender that wants to keep the value wants a copy, and "declare it with `let`" is the wrong advice here); a **parameter not declared `give`** is refused with `ParamNeedsGive` (see "Ownership must flow through the call" below — this refusal REPLACES the `:4611` share-only refusal at every give site, so the share refusal is retired into it rather than extracted); otherwise the binding is marked consumed (`scope.consume`, cause `Sent { channel }`). (The other `scope.consume` site, `check.rs:1498–1512`, is the `background` statement-form give-INFERENCE path — it carries no refusals, because its candidates are already liveness-filtered — so it is not the model here; an earlier draft had these two labels swapped.)
- **A `.copy()` postfix expression** (`PostfixOpKind::Copy` on any receiver) — admitted with nothing to consume: the copy is a fresh allocation nobody else holds. This is what makes every `.copy()` WHAT-INSTEAD in this section executable — a rule that refused non-identifiers uniformly would refuse its own advice.
- **An array or map literal** — admitted with nothing to consume, for the same reason: fresh by construction.
- **Anything else** — a field access (`wire.send(bucket.rows)`), an index (`wire.send(matrix[0])`), a call result (`wire.send(makeRows())`), a `wait`/`background` expression — is a compile error, `SendPayloadNeedsCopy` (text in "Teaching text"). There is no binding to consume, so the sender side may still reach the value through the field, the index, or whatever the call returned (a function may return an alias of its own `share` parameter), and the channel would then free a value the sender still holds. The ident-only consume never saw these forms; the second review round's soundness claim silently assumed them away.

After the send, any read of a consumed binding reaches the existing consumed-read site (`check.rs:3622–3631`) — the ONE place a use-after-give is reported today, and it stays the one place.
...
1. **A parameter binding sent on an owned-heap channel must be declared `give` on its enclosing function.** The send arm reads the binding's `ScopeEntry`: `is_param && param_ownership != Some(Give)` → compile error `ParamNeedsGive`. With `give` declared, the EXISTING machinery does the rest with no new code path: every call site — `wait producer(wire, rows)`, a plain call, a UFCS dot-call, a generic call — reaches `check_arg_ownership`'s Give arm (`:4599–4619`) and consumes the caller's binding; every `background` spawn site reaches it too, because the `Expr::Background` arm (`check.rs:3283`) runs `infer_expr(inner)` on the spawned call (`:3302`) and that call is `check_user_fn_call`, whose argument loop is the `:4799` site. The statement-form liveness inference (`:1443–1515`) still runs and records Give or Copy for codegen; for a declared-`give` parameter the binding is already consumed when it runs, so a program in which the spawner reads the binding afterward does not compile (the consumed-read site fires) and a program in which it does not read it records Give — the two never disagree on a program that builds.
2. **A non-binding owned-heap payload is refused** (`SendPayloadNeedsCopy`, the fourth form above) with the `.copy()` WHAT-INSTEAD this section already ships.

**How far the `give` obligation transits — decided here, not left for Phase 4.** All the way: every function between the value's owner and the send declares `give` on that parameter. A relays to B relays to C, and C sends: C's parameter must be `give` (guard 1); B passes its own parameter to C's `give` parameter, so B's parameter must be `give` too — otherwise B's caller keeps a live binding for exactly the reason the program above does; and so on up to the frame that owns the value as a local. This is enforced at ONE site: `check_arg_ownership`'s Give arm, whose current `else if !entry.is_consumed { consume }` branch (`:4617`) silently consumes a bare or `lend` parameter passed to a `give` parameter — that silent consume IS the relay hole, and it becomes `ParamNeedsGive` (`is_param && param_ownership != Some(Give)` → error; else consume). Because the send arm delegates to the same helper's rule, "sent into a channel" and "given to a `give` parameter" are one rule with one diagnostic and a `{act}` slot, not two rules kept in sync by hand. **The compile error names the immediate frame only** — the function whose parameter is given away without `give` — because that is the only frame whose fix is local (one word in one signature) and the only frame typeck has in hand; the WHY tells the user the word travels up the chain as far as the value does, so a relay chain of N hops surfaces one frame per compile, each fix one word, each pointing at the caller. That is an honest cost, named here: the alternative (report the whole chain at once) needs a callee-before-caller ordering typeck does not have, and the diagnostic's WHY makes the direction of the next fix unambiguous. Today's fixture corpus: `give`-parameter functions are called from `entrypoint` with locals (`v0_3_m4_p3_cross_give.ynz`, `v0_3_m3b_p4_give_arg_sequenced.ynz`, `m4_player.ynz`, `pirates-roster/entrypoint.ynz:350,1108`); Phase 4 confirms zero relay-through-bare-parameter instances by compiling the corpus, not by inspection.

**The alternative weighed: infer `give` for a bare parameter the body gives away (Golden Rule 4's direction).** Rejected, for three reasons. (1) `check_arg_ownership` reads the callee's DECLARED `param_ownerships` from its signature, available before any body is checked; an inferred `give` would require checking every callee body before every caller's call sites — a whole-program ordering with a fixpoint under mutual recursion that the checker does not have. (2) The inference would silently change the CALLER's program — its binding becomes unusable — from an edit inside another function's body, possibly another file; [`inference.md`](../../../.claude/rules/inference.md) puts ownership words in signatures precisely so the contract is visible at the definition, and treats ownership-transfer inference as cautionary-class, which would need the muted hint at every affected call site. (3) Golden Rule 5: `give` in the signature makes the transfer a compile-time fact every reader of the signature sees. Patrick ruled for the explicit word. `share` and `lend` parameters are refused by the same rule: they are explicit statements that the caller keeps the value, so giving it away contradicts the declaration.

**What this closes in Future Requirement #9.** FR#9 names `background producer(wire, table)` + `wire.send(table)` on a `channel<map<..>>` as "this door through the channel" — a `map` bg arg is passed ALIASED (un-cloned) by `prepare_bg_arg_for_ctx`, so the task would send the PARENT's allocation while the parent's binding stayed live. Guard 1 closes that instance: the send requires `give table` on `producer`, and `give` consumes the parent's `table` at the spawn (through `check_arg_ownership`, as above), so the parent can no longer read the aliased map; the channel or its receiver is the allocation's only holder, the ladder has no descriptor for an un-cloned arg (nothing to double-free), and the parent's own frame never frees it (no drop pass). The pass-a-`give`-map-and-send shape is therefore asserted CORRECT by Phase 4's `channel<map>` fixture, not RED-pinned. What guard 1 does NOT close is FR#9's non-channel door — `bucket.add(rows)` storing a ladder-owned clone into an aliased container — which stays RED-pinned under FR#9; the plan's FR#9 text is updated to say exactly this rather than leaving the channel door overstated. The one remaining source-level blind spot is the pre-existing alias-by-`let` (`let alias = rows; wire.send(alias)`), already recorded under "`share` read-only enforcement is best-effort" and in the "Two mechanisms" table below — unchanged by this round. **The consumption-cause change is a signature change, not a flag**: `ScopeEntry.is_consumed: bool` (`scope.rs:31`) becomes `consumed: Option<ConsumedBy>` with `enum ConsumedBy { Given, Sent { channel: String } }` (the channel binding's name fills `{channel}` in the diagnostic); `Scope::consume(name)` (`scope.rs:85`) gains the cause parameter; the seven `ScopeEntry` constructors (`check.rs:642`, `:1406`, `:1623`, `:1660`, `:1723`, `:2803`, plus the struct literal in `scope.rs`) initialize `None`; the two `!entry.is_consumed` guards (`check.rs:1510`, `:4617`) become "not already consumed by ANY cause"; and the consumed-read site selects the `Consumed` or `ConsumedBySend` template by the cause. Phase 4 must not bolt an `Option<String>` beside the bool — one field, one enum. The fix for a sender that still needs the value is `wire.send(rows.copy())`.

**Which element types.** The bite is exactly the owned-heap element set — the types for which the channel registers drop glue at construction: `array<T>` and `map<K, V>`. `channel_drop_glue` has exactly those two non-null arms, and typeck admits only `int`/`float`/`bool`/`string`/`array`/`map` as element types. `int`, `float`, `bool` are value bits: the send copies them and the binding stays usable, so every existing `ch.send(v)` on a primitive channel is unchanged. `string` is immutable, immortal bytes with no glue: sharing the pointer is sound and nothing ever frees it, so a sent string binding also stays usable. The predicate "is this element type owned-heap?" must be ONE definition both crates read — typeck for the consume decision, codegen for the glue arm — per [`authoritative-derivation.md`](../../../.claude/rules/authoritative-derivation.md). **It is `Option<enum>`, not a bool and not a three-variant enum**, because codegen needs the drop function, not a yes/no, AND because the absence case must be unrepresentable inside the glue-bearing set: `pub enum ChannelElemDrop { Array, Map }` with `pub fn channel_elem_drop(elem: &Type) -> Option<ChannelElemDrop>` in `ynz_typeck::types`. Typeck consumes on `is_some()`; `channel_drop_glue` (`emit.rs:15509`) replaces its two-arm `match &elem` with `match channel_elem_drop(elem) { None => null, Some(kind) => <fn pointer> }` where the inner `match kind` is EXHAUSTIVE and its arms are typed as a function value (`Array → ynz_array_drop`, `Map → ynz_map_drop`) — never a nullable pointer. A three-variant `{ None, Array, Map }` was the second draft's shape and is rejected: an exhaustive match only guarantees an ARM exists, not that the arm registers glue — `Number => cg.ptr().const_null()` compiles, and since typeck consumes on "not `None`", that is precisely the silent divergence the enum exists to forbid (a consumed binding whose payload nobody frees). With `Option`, every variant reachable from `Some(..)` must produce a real drop function or the arm does not type-check. Neither existing predicate fits: `is_trivially_copyable` (`types.rs:242`) classifies `string` wrong for this purpose, and `is_mutable_heap_type` (`independence.rs:751`, private) returns `false` for `number`, which fr12 would need in the set. `check_channel_construction`'s `elem_supported` list (`check.rs:4286–4297`) is then a THIRD listing of element kinds; Phase 4 derives it from the same function (`supported = value-bits ∪ string ∪ channel_elem_drop(elem).is_some()`) or pins it with a parity test over every `Type` variant — never a third hand-maintained list. When fr12 adds element kinds, they join by adding an enum variant, and the exhaustive inner match tells codegen where the glue is missing — and cannot be satisfied with a null.

**What this makes sound — and its precondition.** With the sender's binding consumed, AND every parameter between the value's owner and the send declared `give` (so each caller's binding is consumed in turn), the buffered value has exactly one source-level holder at every moment: the owner until the send lands, then the channel, then the receiver. Teardown glue frees a value nobody else holds. The receiver's `maybe<array<T>>` is the sole reference. The claim holds for a payload reached through named bindings and `give` parameters — a task-built value, a heap-CLONED `background` argument (an `array<int|float|bool|shape>` — ladder-owned, and released from the ladder at the send by the shipped runtime protocol in "Two mechanisms, one rule" below), and now also an ALIASED `background` argument (`map<K, V>`, `array<pointer-elem>`, union — `prepare_bg_arg_for_ctx`'s un-cloned fall-through) **provided it arrives through a `give` parameter**, because the `give` consumes the parent's binding at the spawn and the parent can no longer reach the aliased allocation (see "What this closes in Future Requirement #9" above). Without `give`, such a parameter cannot be sent at all (`ParamNeedsGive`). What stays outside the claim: FR#9's non-channel door (`bucket.add(rows)` into an aliased container — RED-pinned by `bg_arg_alias_container_add_red.ynz`), and the alias-by-`let` blind spot already recorded below. Phase 4's `channel<map<K, V>>` fixture therefore covers BOTH shapes — a map built inside the sending task, and a map passed as a `give` bg arg and sent — as correct programs, plus the no-`give` variant as a gallery trigger.

**Four new compile-time diagnostics.** Use-after-send (`ConsumedBySend`); the two ownership-flow guards from "Ownership must flow through the call" (`ParamNeedsGive` — which also replaces the existing share-parameter refusal at `check.rs:4611` — and `SendPayloadNeedsCopy`); and the handle-spawn channel-binding guard (`HandleChannelArgNeedsBinding`, from "The gap that decision leaves" below). Full text for all four in "Teaching text". All four join the v0.3-M8 plan's Invariants → Teaching list and Phase 9's error-gallery obligation.

**Fixture gap Phase 4 must close.** There is **no `channel<array<T>>` or `channel<map<K, V>>` end-to-end fixture anywhere in the tree** — the only non-primitive channel fixture is `v0_3_m4_channel_construct.ynz:14` (`channel<string>`, construction only). The give semantics, the closed-arm free, and the teardown-glue path for owned-heap elements have never been executed by a `.ynz` program. Phase 4 authors one RED→GREEN fixture per element kind (array, map) covering: send-then-receive round-trip; send-then-read (the compile error, in the gallery); send-after-close (the freed payload, with alloc/free parity via `YNZ_ALLOC_COUNTER_OUTPUT`); drop-with-buffered-element; **the ownership-flow class** — the reviewer's `wait producer(wire, rows)` program above, once with `give rows` (the parent's later read is `ConsumedBySend`/`Consumed` at the call site — gallery) and once without (`ParamNeedsGive` — gallery); a two-hop relay (`A → B(give) → C(give)`, correct; and `B` without `give`, the error naming `B`); a `give`-map bg arg passed and sent (correct, alloc/free parity); and the three refused payload forms (field, index, call result → `SendPayloadNeedsCopy`) beside the three admitted ones (ident, `.copy()`, literal).
...
| **What it cannot see** | Whether the parameter is ladder-owned (same function body, spawned or `wait`ed); aliases (`let alias = rows` — the typeck consume tracks names, and the ownership model's alias hole is already recorded under "`share` read-only enforcement is best-effort"). It DOES see a parameter's provenance now — but only because `ParamNeedsGive` forces the `give` word that makes every caller's consume happen through the same helper | The program's future reads |
...
**Where the shipped code and this design genuinely disagree — the CLOSED-first-poll path.** The shipped send core says, in its doc comment and in test `ladder_is_untouched_when_the_channel_does_not_take_ownership` case (c): *"A `CHANNEL_CLOSED` result on a FIRST poll never releases: the value was not taken, the sender still owns it."* On today's tree that is correct — nothing consumed the binding, so the sender can still read it. **This design says `send()` gives its payload on EVERY outcome** — typeck consumes before the outcome is knowable, so after Phase 4 a refused payload has no source-level owner and MUST be freed by the send path, or it leaks. The first draft put that free (P2-3) in codegen's two closed arms via the glue. With the ladder in view that is wrong: a codegen-side free of a ladder-owned clone on CLOSED, plus a runtime that does not release on CLOSED, is a DOUBLE FREE at task retire. **Resolution: P2-3's free moves into the runtime's CLOSED-first-poll path — and there are THREE first-poll CLOSED arms, not two.** They are: (i) the new `None`-under-the-sender-lock arm; (ii) `try_send → Closed` (`channel.rs:503`); (iii) `try_send → Full`, then the freshly-boxed endpoint future's FIRST poll returning `Ready(Err(()))` (`channel.rs:554`) — the value was moved into that future, the future is dropped, and only the `value_bits` mirror remains. The second draft listed two and would have shipped (iii) as the P2-3 leak again. All three collapse into ONE fallthrough: a local `refuse_closed(chan, value_bits)` that, when `drop_glue.is_some()`, calls `release_taken_value()` and then `glue(value_bits)`, and returns `CHANNEL_CLOSED` — exactly what the shipped re-poll-CLOSED arm already does for an orphaned parked payload. No arm returns `CHANNEL_CLOSED` except through it, so a fourth closed outcome added later inherits the free or fails review for not calling it. `channel_send_poll_guarded` then decides the payload's fate on ALL outcomes in one function: READY → the channel owns it; PENDING → the pending entry owns it; CLOSED (any of the three) → freed here, nobody owns it. Codegen's `conduit_closed1`/`conduit_closed2` arms build the error value and free NOTHING. Phase 4 flips the shipped doc comment and test case (c) to the new contract and adds the parity gate for a CLOSED first poll of a ladder-owned payload. This is the one place the shipped code assumes the pre-consume world; it is called out here so it is changed deliberately, not discovered as a double free.
...
- **FOUR `[[diagnostic_template]]` entries.** `kind_name = "ConsumedBySend"` — the use-after-send compile diagnostic (text in "Teaching text" below); canonical and reusable (it fires for every owned-heap send followed by a read, with only `{name}`/`{channel}` varying), which is exactly the template kind's criterion. `kind_name = "ParamNeedsGive"` — a parameter given away (sent into an owned-heap channel, or passed to a `give` parameter) without a `give` declaration; `{name}`/`{fn}`/`{modifier}`/`{act}`/`{type}`/`{copy_form}` varying; it REPLACES the inline share-parameter refusal at `check.rs:4611–4616`, whose text is retired (Phase 4 updates whatever gallery line or snapshot asserts the old share-refusal wording — `v0_3_m4_errors.ynz` is the first place to look). `kind_name = "SendPayloadNeedsCopy"` — a non-binding, non-copy, non-literal owned-heap payload at a send; `{channel}`/`{expr}`/`{type}` varying. `kind_name = "HandleChannelArgNeedsBinding"` — the handle-spawn guard (text below); same criterion, `{callee}`/`{expr}` varying. While registering it, Phase 4 reconciles a pre-existing drift found on the same site: the existing `Consumed` template (`features.toml:1599–1603`, "cannot be used again" / "Use `.copy()` before giving a value away…") and the code that emits it (`check.rs:3624–3629`, "cannot be used here" / "Create a new value or use `.copy()`…") disagree in both WHAT and WHAT-INSTEAD; the emitting site becomes the one consumer of both templates, selected by consumption cause, so the registry is the source for both.
...
> **WHY**: A channel hands the value to whichever task receives it, and that task may already be reading or changing it by the time this line runs. If both sides kept the same value, two tasks could change it at once and neither would know. So `send()` gives `{name}` away, and the compiler refuses to build any line that reads it afterward. Nothing happens at runtime — `{name}` is not cleared or set to `none`; the program simply does not compile until this read is placed above the `send()` or the send takes a copy.

(The WHY must describe the compile-time refusal at the consumed-read site (`check.rs:3622–3631`), never a runtime state of the binding — an 18-year-old JS developer reads "empty" as `null`/`undefined` at runtime; corrected in fix round 2, `git log --grep=m8-p1-fix2`.)
...
The cannot-give-const refusal is **extracted** from `check_arg_ownership`'s inline `format!` (`check.rs:4602–4607`) into a helper with a caller-supplied WHAT-INSTEAD, and the send arm supplies `Send a copy instead: {channel}.send({name}.copy()).` — the existing "declare it with `let`" advice is wrong for a send, where the caller keeping the value is the whole point. The share-cannot-be-given refusal (`:4611–4616`) is NOT extracted: it is the `share` instance of `ParamNeedsGive` below and is retired into that template. `.copy()` is real for both element kinds the rule covers once `map`'s ships (above).

**New compile-time diagnostic — a parameter given away must be declared `give`** (`ParamNeedsGive`; fires in two places that are one rule: `check_conduit_method_call`'s `send` arm when the payload identifier's `ScopeEntry` has `is_param && param_ownership != Some(Give)`, and `check_arg_ownership`'s Give arm (`check.rs:4599–4619`) under the same test — the branch that today silently consumes a bare or `lend` parameter at `:4617`, and the share-only refusal at `:4611`, both become this. `{name}` is the parameter, `{fn}` its function, `{type}` its declared type, `{modifier}` one of `has no ownership word` / `is declared \`share\`` / `is declared \`lend\``, `{act}` one of `sent into \`{channel}\`` / `given to \`{callee}\``, `{copy_form}` the matching `{channel}.send({name}.copy())` / `{callee}({name}.copy(), …)`):
...
> **WHY**: A parameter without `give` still belongs to the caller — the caller's own binding stays usable after `{fn}` returns. If `{fn}` could hand that value onward anyway, the caller and the new holder would both have it, and when the new holder is done with it and frees it, the caller would be reading memory that is already gone. `give` in the signature makes the caller's binding unusable after the call, so the value has exactly one holder at every step. If the caller's value is itself a parameter, that function needs `give` too — the word travels up the chain as far as the value does.

Worked example of the rendered form, for the gallery (the reviewer's program from "Ownership must flow through the call"):

```ynz
function producer(wire: channel<array<int>>, rows: array<int>) -> nothing {
  wire.close()
  let sent = wire.send(rows)
  // COMPILE ERROR: `rows` is a parameter of `producer` that has no ownership word, but this line
  //   gives it away — it is sent into `wire`. Only a `give` parameter can be given away.
  //   Declare it `give` in `producer`'s signature — `function producer(give rows: array<int>, …)` —
  //   so every caller hands the value over for good. If `producer` should leave its caller's value
  //   usable, give away a copy instead: `wire.send(rows.copy())`.
  //   Why: a parameter without give still belongs to the caller — the caller's own binding stays
  //   usable after producer returns. If producer could hand that value onward anyway, the caller and
  //   the new holder would both have it, and when the new holder is done with it and frees it, the
  //   caller would be reading memory that is already gone. give in the signature makes the caller's
  //   binding unusable after the call, so the value has exactly one holder at every step. If the
  //   caller's value is itself a parameter, that function needs give too — the word travels up the
  //   chain as far as the value does.
}
```

With `give rows` declared, the same program's `print(rows.count())` in `entrypoint` is the existing use-after-give error at the consumed-read site — the parent's binding was consumed by `wait producer(wire, rows)` through `check_arg_ownership`, exactly as if `rows` had been given to any other `give` parameter.

**New compile-time diagnostic — a sent payload must be a binding, a copy, or a literal** (`SendPayloadNeedsCopy`; fires in `check_conduit_method_call`'s `send` arm when the element type is owned-heap and the payload expression is not `Expr::Ident`, not a `.copy()` postfix, and not an array/map literal; `{channel}` is the channel binding, `{expr}` the payload's source text, `{type}` the element type):

> **WHAT**: `{channel}.send({expr})` sends a `{type}` that is not a named binding, so the compiler cannot tell who else still holds it.
> **WHAT INSTEAD**: Send a copy: `{channel}.send({expr}.copy())`. If `{expr}` is a call that builds a brand-new value, you can also bind it first and send the binding — `let rows = {expr}` then `{channel}.send(rows)`. A field or an index is not new — copy it.
> **WHY**: Sending a `{type}` hands it to whichever task receives it, and that task will eventually free it. For a named binding the compiler marks the binding as given away, so nothing on this side reads it afterward. For `{expr}` there is nothing to mark — the value may still be reachable through the field, the index, or another binding — so two tasks could hold the same value and one of them would free it under the other. A `.copy()` belongs to nobody else, and a `let` gives the compiler a name it can mark.
...
let sent = wire.send(bucket.rows)
// COMPILE ERROR: `wire.send(bucket.rows)` sends a `array<int>` that is not a named binding, so the
//   compiler cannot tell who else still holds it.
//   Send a copy: `wire.send(bucket.rows.copy())`. If `bucket.rows` is a call that builds a brand-new
//   value, you can also bind it first and send the binding — `let rows = bucket.rows` then
//   `wire.send(rows)`. A field or an index is not new — copy it.
//   Why: sending a array<int> hands it to whichever task receives it, and that task will eventually
//   free it. For a named binding the compiler marks the binding as given away, so nothing on this side
//   reads it afterward. For bucket.rows there is nothing to mark — the value may still be reachable
//   through the field, the index, or another binding — so two tasks could hold the same value and one
//   of them would free it under the other. A .copy() belongs to nobody else, and a let gives the
//   compiler a name it can mark.
```
...
### Open at sign-off (Patrick's decisions — listed discretely, not folded into the design narrative)

The section header says "awaiting sign-off" rather than "locked" because three items below are decisions, not settled facts. Each is designed to a recommended answer so the sign-off can be a yes; none is assumed.

1. **fr12's disposition — OPEN, deliberately undecided by every fix round.** Does `channel<number>` decimal128 marshalling land as a Phase 4 step, or as its own FRAGO'd design step? See "fr12 — separable, and why". The close design constrains it only by the shared owned-heap predicate.
2. **`HandleChannelArgNeedsBinding` as a compile ERROR rather than a warning.** Designed as an error (handle form only) for the reasons in "The gap that decision leaves": a warning leaves the hang class live for one spawn shape while the fix costs one `let`, and Golden Rule 5 says wrong is caught at compile time. A reviewer flagged that the previous header presented this as settled; it is Patrick's call and is surfaced here as one.
3. **`give` on parameters — a real language-ergonomics change, confirm knowingly.** (a) A parameter sent on an owned-heap channel must be declared `give` (`ParamNeedsGive`); (b) the SAME rule now applies at `check_arg_ownership`'s Give arm, so a bare or `lend` parameter can no longer be passed to a `give` parameter silently (today it is consumed without a word at `check.rs:4617`) — every relay frame between an owner and a give declares `give`, one frame reported per compile; (c) the existing share-parameter refusal is retired into the new template; (d) the alternative — inferring `give` for a bare parameter the body gives away — was weighed and rejected (ordering/fixpoint, silent change to the caller's program, signatures-carry-ownership per `inference.md`). Patrick ruled for the guard in fix round 3; the sign-off confirms the transit rule (b) and the retirement (c), which are consequences the ruling did not spell out. Also under this item: (e) `SendPayloadNeedsCopy` refuses field/index/call-result payloads and admits identifiers, `.copy()`, and literals — a call result therefore needs a `let` or a `.copy()`, which is Golden Rule 7's step-by-step form but is one more line than today.
