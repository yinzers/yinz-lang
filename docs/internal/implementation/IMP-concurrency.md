...
Developers can always force an independent copy explicitly: `background process(data.copy())`. There is no body-level `.give` form (ownership words are signature-only — [`IMP-ownership.md`](IMP-ownership.md) read-first note; `PostfixOpKind` is `Copy | Freeze`); the give direction is what the compiler already does for a value the caller never reads again. (An earlier revision of this line named `data.give` and bare `data.copy`; corrected in v0.3-M8 Phase 2 when the Auto-Arc override analysis checked it against the parser.)
...
### fr12 — `channel<number>` decimal128 marshalling (designed v0.3-M8 Phase 2 per FRAGO 009; Phase 4 implements; awaiting Patrick's sign-off with the rest of Phase 2)
...
### Sign-off record and what moved

**Phase 1 was signed off by Patrick on 2026-09-03, narrowed by FRAGO 008** (`audit.md` SIGN-OFF record): the close mechanism, the name, `receive()` → `maybe<T>`, idempotency, wake-all, in-flight completion, drop-unchanged, the `refuse_closed` collapse, P2-3's free in the runtime, `Option<ChannelElemDrop>`, `HandleChannelArgNeedsBinding` (a hard compile ERROR — FRAGO 009 ruling 2), and the four-field auto-close deferral. The three items the earlier "Open at sign-off" list carried are disposed as follows:

1. **fr12's disposition** — RULED (FRAGO 009): designed in Phase 2, in the "fr12" subsection above; Phase 4 implements. Its `number`-stays-copy-through decision and the `transfers_source()` split of `ChannelElemDrop` await Phase 2's sign-off together with the transfer rule.
2. **`HandleChannelArgNeedsBinding` as an ERROR** — RULED (FRAGO 009): error, as designed.
3. **The `give` requirement on parameters, its transit, and the payload-form set** — MOVED to Phase 2 (FRAGO 008) and re-derived there: [`IMP-ownership.md`](IMP-ownership.md) "Transfer — Who Else Holds This Value". What Patrick confirms at Phase 2's sign-off is listed in that section's plan block (the v0.3-M8 plan, Phase 2), not here.
