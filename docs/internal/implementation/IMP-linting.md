---
name: "IMP-linting"
description: "Design decisions for Yinz linting and build tooling — why the compiler itself is the linter (no separate lint tool), the three-tier diagnostic model, and yinz.toml lint configuration."
tags:
  - "yinz-compiler"
...
User spec: [`docs/reference/REF-linting.md`](../../reference/REF-linting.md), [`docs/reference/REF-tooling.md`](../../reference/REF-tooling.md). Full compiler design: [`docs/internal/implementation/IMP-compiler.md`](IMP-compiler.md). Teaching mission alignment: [`docs/reference/REF-teaching-mission.md`](../../reference/REF-teaching-mission.md).
...
| `out-of-precision-literal` | `number<N>` with N > 4096 (the cap from [`docs/internal/implementation/IMP-numeric-types.md`](IMP-numeric-types.md)). |
...
| `prefer-fixed-when-immutable` | `array<T>` that never calls `.add()` or `.remove()` — compiler auto-promotes to `fixed<T>` codegen silently; lint suggests writing `fixed<T>` explicitly for source clarity. Hybrid model — see [`docs/internal/implementation/IMP-collections.md`](IMP-collections.md) "Auto-promotion" section |
...
Every lint diagnostic — errors, warnings, suggestions — follows the WHAT / WHAT-INSTEAD / WHY format defined in [`docs/reference/REF-teaching-mission.md`](../../reference/REF-teaching-mission.md):
...
**Not a golden rule**: The 13 golden rules describe the language from the user's perspective. Compile speed is a compiler implementation goal. Full design: [`docs/internal/implementation/IMP-compiler.md`](IMP-compiler.md).
