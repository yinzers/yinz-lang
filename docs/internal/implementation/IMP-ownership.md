...
| `for` loop variable, and every name a `for` destructure binds (`destructure_pattern`, `map_destructure_pattern`) | `Stmt::For` | none (frame-scoped, and `Assign` to a loop variable is already a compile error, `check.rs:2444`) | `Cell` (a cell of the iterated value; never transferable); joins the iterated root's class |
...
3. A `background` argument the spawn gives away — **and this sink is the liveness INFERENCE, not a `check_transfer` call.** The inference at `check.rs:1443–1515` chooses `Give` or `Copy` for a binding the user never annotated; nothing it chooses can be reported as an error against a `give` the user did not write. Its rule, restated class-aware: for a `Whole(name)` argument, infer `Give` iff the binding's origin is `Owned` or `Param(give)` AND no member of its alias class is read in any remaining statement; otherwise infer `Copy`. A `Cell`, `Reaches`, `Unknown` or `Param(share|lend|bare)` origin therefore declines to `Copy` silently — `for (row in matrix) { background producer(wire, row) }` is a correct program today (probe: prints `3`, `matrix` intact afterwards) and stays one; codegen already copies by TYPE regardless of the recorded variant (`prepare_bg_arg_for_ctx`; `bg_inferred` is read only by the inlay-hint pass, `inlay_hint_passes.rs:765`), so the decline changes no runtime behavior — it only stops typeck from consuming a name the frame does not own, and makes the muted hint say "copied", which is what happens. The `Give` consume routes through the class, so a name that aliases the given binding is consumed with it — and because liveness is asked of the CLASS, `let other = rows; background f(rows); other.count()` infers `Copy` rather than consuming `other` under a program that runs correctly. The spawned call's own DECLARED `give` positions are sink 2 (`TransferNeedsCopy` on a loop cell there is correct: the callee's author wrote the word), because the `Expr::Background` arm runs `infer_expr` on the call.
