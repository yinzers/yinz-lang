...
Inline: `// shared by reference count with {n} tasks — read-only` — `{n}` is the group's member count, read from the same `BgOwnership::Arc` record codegen emits from. The hover is static registry markdown (`ynz_registry::lsp_inlay_hint_hover_for` substitutes nothing), so the registry text says "the background tasks spawned here" where the draft below writes `{n}`; the count is in the label. The draft, tied to the actual group:
...
- `array`/`map` values and shapes with pointer-cell (`array`/`map`/`maybe`/union/nested `shape`) or `number` fields (a different sharing substrate; `arc_shareable` is the floor).
