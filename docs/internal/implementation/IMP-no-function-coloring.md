...
4. Cross-thread shared state crosses a `background` boundary via auto-inferred reference-counted sharing — **live since v0.3-M8 Phase 5** for a GROUP of spawns: two or more spawns in one block passing the same whole shape binding, nothing changing it between them, every spawned function proven read-only (`effective_ownership`), and a shape of `int`/`float`/`bool`/`string` fields share ONE block (`ynz_arc_new` at the first spawn, `ynz_arc_clone` per task, the caller's transient released after the last spawn, each task's reference released at retire). A single spawn, a spawn group straddling a suspension point, a spawn inside a loop, and pointer-carrying or `number`-carrying values still cross via the independent-copy path (the compiler infers `.copy`) — correct and race-free; that residual is what the registry entry `auto-arc-codegen-emission` now names. The IDE shows the `auto_arc` muted hint at every member spawn (normal muted style; the cautionary red tint stays under `auto-arc-cautionary-tint` — the caution is in the hover's words). The sharing topology, the beneficial-emission condition, the read-only proof, and the override analysis are specified in [`docs/internal/implementation/IMP-ownership.md`](IMP-ownership.md) "Auto-Arc — Sharing Topology Across `background` Boundaries" (v0.3-M8 Phase 2 design, Phase 5 implementation); the transfer rule for values that LEAVE a frame is the same document's "Transfer — Who Else Holds This Value".

   **P2-6 disposition (the 2026-07-04 concurrency-release audit's "auto-Arc unwired" finding): CLOSED by v0.3-M8 Phase 5.** The emission above is live; the topology is specified (`IMP-ownership.md` "The topology, decided" — (B), one shared copy, N task references, caller keeps its original); `auto-arc-codegen-emission` (`[[deferred_language_feature]]`) is NARROWED to the residual named above rather than deferring the feature, and `auto-arc-cautionary-tint` (`[[deferred_tooling_feature]]`) still defers only the hint's red-tint styling. Anchor: the v0.3-M8 plan's `audit.md` entries `m8-p5-20260904-a1` and `m8-p5-fix2-20260904`; `git log --grep=m8-p5`.
...
**Implementation milestone**: SHIPPED-DEFERRED v0.3-M4; **re-deferred with evidence v0.3-M8 Phase 7 (Branch B)** — the RUNTIME half is live and substrate-proven: `ynz_handle_free` (`crates/ynz-runtime/src/handle.rs`) aborts the child at its next suspension point via Tokio abort, and the frame + arg-copies + shared-channel refs + the Arc-group refs + the R8 buffer are each freed exactly once (safe-drop: no use-after-free, no leak, alloc=free-gated). The LANGUAGE half — codegen automatically calling `ynz_handle_free` when a handle binding's scope ends (`ynz_handle_free` is declared in codegen's `runtime_decls.rs` but NEVER emitted — zero call sites, pinned by `crates/ynz-driver/tests/v03_m8_handle_scope_pin.rs`), the cancellation surfacing inside the child as a typed `errors` value, and any explicit `.cancel()` API — is NOT implemented.

**Shipped semantics (current state, probed v0.3-M8 Phase 7)**: a task whose handle goes out of scope — at block end, loop-iteration end, or the return of the function that bound it, whether that function is a plain function, a suspending one whose frame its caller frees, or itself a `background` task — runs to completion while `entrypoint` is still running; it is never stopped by the scope exit. When `entrypoint` returns, `ynz_rt_shutdown` stops every still-running task at its NEXT suspension point (a child mid-`wait sleep` at that moment does not resume; its drop ladder still frees its frame, so alloc=free holds). The un-freed handle object is one small Box per spawn (plus its own reference to the child's command channel, if any), reclaimed at process exit.

**Why the M8 investigation re-deferred rather than shipped (the recon the phase existed to do):** the one cleanup dispatch in the tree — `SpawnStateFnFuture::drop`'s kind arms in `crates/ynz-runtime/src/runtime.rs` — is a spawned TASK's retirement ladder: it walks that task's own frame's `BgArgDropEntry` descriptors when that task retires. A handle binding's scope exit is a PARENT-frame event at a parent-side time (a block edge, a loop back-edge, a `return`, a state-machine frame's retirement through the caller's `free_frame`), and no code path releases ANY local of any type on that edge today — `emit.rs` emits `ynz_array_drop`/`ynz_free`/`ynz_channel_free` only inside task-side arg-free glue (`emit_bg_arg_frees`), the channel element-glue table (`channel_elem_drop`), and the spike trampoline's staged cell. So there is nothing for a handle arm to plug into: a handle-only release would have to build the per-scope-edge release enumeration the general pass needs verbatim, then run beside it — the forked second mechanism this section's own parenthetical warned about — and would make "leaving the scope" mean *stop the task* for a handle while the array, map, channel and `maybe` cell bound beside it at the same scope exit stay held (the per-type inconsistency [`.claude/rules/auto-promotion.md`](../../../.claude/rules/auto-promotion.md) bans). Registering the handle in the parent's ladder was weighed too and rejected: only a parent that is itself a `background` task has a ladder, and even there it fires at the parent's retirement, not at the binding's scope exit — structured task-tree cancellation for nested spawns only, a different semantics from the locked model. Four-field re-deferral in the [v0.3-M8 plan's Future Requirements #3](../../../.claude/planning/active/2026-07-04-v0-3-m8-concurrency-completion/plan.md); registry entry `background-handle-cancel-injection` (updated, not retired); trigger: the drop-story milestone's scope-exit release pass — handles join it as one arm and the pin tests flip — OR a real workload needing to stop a running task first. Evidence record: that plan's `audit.md` entry `m8-p7-20260904-a1`.
