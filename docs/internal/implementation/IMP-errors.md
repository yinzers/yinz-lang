---
name: "IMP-errors"
description: "Design rationale for Yinz's 'errors' keyword-based error handling (no try/catch, no Result types) and its flow-sensitive auto-propagation rules."
tags:
  - "yinz-compiler"
...
User spec: [`docs/reference/REF-errors.md`](../../reference/REF-errors.md)
...
This uses M6's narrowing infrastructure extended with a new "errors-capable" narrowing fact (parallel to `maybe<T>`'s exists-fact). See [`docs/internal/implementation/IMP-narrowing.md`](IMP-narrowing.md) for the added rows.
...
The `errors` keyword, flow-sensitive auto-propagation, and cascades are scheduled for **M7** ([`docs/internal/scratchpad/SCRATCH-stdlib-database.md`](../scratchpad/SCRATCH-stdlib-database.md) is already written against this contract). When implementing M7:
...
- See [`docs/internal/scratchpad/SCRATCH-stdlib-database.md`](../scratchpad/SCRATCH-stdlib-database.md) → "Structured Runtime Errors" for the first concrete example of a typed stdlib error this design must support.
