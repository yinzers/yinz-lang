---
name: "ADR-compiler-language"
description: "Records and justifies the decision to write the Yinz compiler in Rust (over Zig, TypeScript, and other alternatives), naming the core crates and toolchain (inkwell, salsa, ariadne)."
tags:
  - "yinz-compiler"
...
The LSP and CLI also share the **SSOT feature registry** ([`registry/features.toml`](../../../registry/features.toml), parsed by `crates/ynz-registry/build.rs`). The registry is the single source of truth for keyword lists, banned-jargon words, primitive intrinsics, type-attached constants, deferred-feature catalog, and IDE muted-hint domains. The CLI reads the registry to generate diagnostics; the LSP reads the same registry to generate autocomplete suggestions and hover docs. Without the registry, "what keywords should autocomplete show?" and "what muted hints should the IDE display?" have no single place to ask — they drift as the feature set grows. See [`docs/internal/implementation/IMP-feature-registry.md`](../implementation/IMP-feature-registry.md) for the schema and migration plan, and [`docs/internal/implementation/IMP-lsp.md`](../implementation/IMP-lsp.md) for how the LSP consumes it.
