---
name: "SCRATCH-stdlib-overview"
description: "Design document for the Yinz standard library. Content here is in planning phase — no implementation yet."
tags:
  - "yinz-compiler"
...
Each module ships in a specific Yinz version per [`docs/reference/REF-mvp-scope.md`](../../reference/REF-mvp-scope.md). The granular versioning means each module gets dedicated design + implementation when its turn comes.

| Module | Ships In | Notes |
|--------|----------|-------|
| `file` + `path` + `directory` | **v0.5** | Tight trio; see [filesystem.md](SCRATCH-stdlib-filesystem.md) |
| `math` | **v0.6** | See [math.md](SCRATCH-stdlib-math.md) |
| `cli` + `env` + `process` | **v0.7** | Tight trio; see [cli.md](SCRATCH-stdlib-cli.md) |
| `json` | **v0.8** | Part of [data.md](SCRATCH-stdlib-data.md) |
| `date` + `duration` | **v0.9** | Tight pair; see [dates.md](SCRATCH-stdlib-dates.md) |
| `db` (database) | **v0.10** | DuckDB + Postgres only at launch; other drivers (MySQL, SQLite, etc.) deferred until after v1.0. See [database.md](SCRATCH-stdlib-database.md) |
| `log` (basic) | **v0.11** | Logging framework v0.23 |
| `random` | **v0.12** | |
| Testing framework | **v0.13** | Language feature, not stdlib (`test` keyword) |
| `regex` | **v0.14** | |
| `request` (outbound HTTP) | **v0.15** | Three-tier API (helpers + builder + raw sockets) — see [network.md](SCRATCH-stdlib-network.md) |
...
| `csv` | **v0.20** | Part of [data.md](SCRATCH-stdlib-data.md) |
| `server` (inbound HTTP) | **v0.21** | Builds on `request` (v0.15); shares `Request`/`Response` types — see [network.md](SCRATCH-stdlib-network.md) |
...
| `ml` / `tensor` | **v2+** | DEFERRED — see [ml.md](SCRATCH-stdlib-ml.md) and `docs/reference/REF-mvp-scope.md#v2--deferred-features` |
| `markets` | **v2+** | DEFERRED — see [markets.md](SCRATCH-stdlib-markets.md) and `docs/reference/REF-mvp-scope.md#v2--deferred-features` |
...
The questions that previously lived in [`docs/internal/scratchpad/SCRATCH-open-questions.md`](SCRATCH-open-questions.md) have been resolved:

- **Package manager design** — RESOLVED. See [`docs/internal/implementation/IMP-packages.md`](../implementation/IMP-packages.md). Ships v0.22.
- **Testing framework** — RESOLVED. Built into the language as the `test` keyword. See [`docs/internal/implementation/IMP-testing.md`](../implementation/IMP-testing.md). Ships v0.13.
...
- **Database (`db` module)** — RESOLVED. Ships v0.10 with **DuckDB + Postgres only**. All other drivers (MySQL, SQLite, MariaDB, MS SQL, etc.) **deferred until after v1.0 launch**. See [`docs/internal/scratchpad/SCRATCH-stdlib-database.md`](SCRATCH-stdlib-database.md).
...
| Candidate | What | Note |
|---|---|---|
| **`encoding`** | base64 (std + URL-safe), hex, URL/percent codec | **Already has a design doc** → [encoding.md](SCRATCH-stdlib-encoding.md). `stdlib-design.md` Rule 8 names base64; `request` v0.15 needs URL-encoding. Slot before/with v0.15. |
...
| **`set<T>`** ✅ COMMITTED 2026-06-13 → **v0.4** | core collections ([`docs/internal/implementation/IMP-collections.md`](../implementation/IMP-collections.md)) — a **language feature**, like `map` (co-ships with the v0.4 linting tier) | A set is `map<K,V>` with keys only: O(1) membership + auto-uniqueness + set algebra. Inherently growable (no "fixed set"). Designed in `collections.md`. Includes `array<T> → set<T>` auto-promotion (membership-only). |
| **`array.unique()`** ✅ COMMITTED 2026-06-13 | core collections ([`docs/internal/implementation/IMP-collections.md`](../implementation/IMP-collections.md)) | The "dedupe a list once" case — a method on `array`, not a type. Ships with `set<T>`. |
