---
name: "SCRATCH-future-string-ptr-len-overhaul"
description: "Deferred design notes on migrating Yinz strings from NUL-terminated C strings to {ptr, len} slices, covering rationale, migration scope, and a target version TBD."
tags:
  - "yinz-compiler"
...
User spec target: [`docs/reference/REF-strings.md`](../../reference/REF-strings.md) (already partially describes the SSO layout that anticipates this overhaul).
...
**SSO (small-string optimization)**: [`docs/reference/REF-strings.md`](../../reference/REF-strings.md) already describes a 23-byte inline SSO layout. That layout's design anticipates `{ptr, len}` — the inline form stores bytes-and-length-and-cap inline; the heap form points-to-ptr + length-and-cap. So SSO is already designed for this overhaul.
...
- **Encoding**: stay UTF-8-only (per [`docs/reference/REF-strings.md`](../../reference/REF-strings.md) and stdlib-design.md Rule 3). No change.
...
- [`docs/reference/REF-strings.md`](../../reference/REF-strings.md) — the user-facing string spec; SSO layout already anticipates this
- [`docs/internal/implementation/IMP-strings.md`](../implementation/IMP-strings.md) — the design rationale for UTF-8 + SSO + locale-invariant case ops
- [`.claude/rules/stdlib-design.md`](../../../.claude/rules/stdlib-design.md) Rule 3 (no platform-default encoding) — overhaul keeps UTF-8 default
- [`.claude/rules/stdlib-design.md`](../../../.claude/rules/stdlib-design.md) Rule 8 (SIMD where available) — `{ptr, len}` enables SIMD-friendly bulk byte ops
