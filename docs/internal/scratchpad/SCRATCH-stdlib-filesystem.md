---
name: "SCRATCH-stdlib-filesystem"
description: "Early scratchpad ideas for a future stdlib file-system module covering file read/write/append and directory operations, all using the errors system."
tags:
  - "yinz-compiler"
...
- **SIMD UTF-8 validation on read** — already mandated by [`.claude/rules/stdlib-design.md`](../../../.claude/rules/stdlib-design.md) Rule 8 (validate decoded bytes via `simdutf`/equivalent, ~0.7 cycles/byte vs ~8 scalar). Cross-referenced here so the v0.5 read path wires it in.
