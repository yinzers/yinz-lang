---
name: "REF-teaching-mission"
description: "Yinz exists not just to be easy to USE, but to actively teach developers to write better code. This is a positioning decision that differentiates Yinz from every other language in its class."
tags:
  - "yinz-compiler"
...
The full protocol lives in [`docs/reference/REF-ide-hints.md`](REF-ide-hints.md) (v0.2 LSP implementation target). The principle is locked into Yinz's identity now so v0.2 doesn't redesign it.
...
Per [`.claude/rules/language-design.md`](../../.claude/rules/language-design.md), every new feature, rule, or message must answer four questions:
...
The teaching mission extends to the spec itself. Per [`.claude/rules/spec-writing.md`](../../.claude/rules/spec-writing.md):
